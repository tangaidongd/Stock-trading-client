...
//lint -esym(552, pSetWindowTheme) suppress "Symbol 'pSetWindowTheme' not accessed : pSetWindowTheme变量已被赋值，但却没有被访问"
...
//目前仅仅对xp起作用
COLORREF CNewMenu::m_clrMenuBk = GetSysColor(COLOR_WINDOW);
COLORREF CNewMenu::m_clrMenuBitmapBk = MixedColor(GetSysColor(COLOR_MENUBAR),m_clrMenuBk);
COLORREF CNewMenu::m_clrMenuBorderBk = GetSysColor(COLOR_HIGHLIGHT);
COLORREF CNewMenu::m_clrMenuItemSelBk = MixedColor(GetSysColor(COLOR_WINDOW),GetSysColor(COLOR_HIGHLIGHT));
COLORREF CNewMenu::m_clrMenuItemSelText = GetSysColor(COLOR_WINDOWTEXT);
COLORREF CNewMenu::m_clrMenuItemNorText = GetSysColor(COLOR_MENUTEXT);
COLORREF CNewMenu::m_clrMenuItemDisableText = GetSysColor(COLOR_GRAYTEXT);
/////////////////////////////////////////////////////////////////////////////
// Helper datatypes
class CToolBarData
{
...
//lint --e{5071} suppress "Size incompatibility, converting 4 byte pointer to 2 byte integral"
...
void MenuDrawText(HDC hDC ,LPCTSTR lpString,int nCount,LPRECT lpRect,UINT uFormat)
{
	...
	TCHAR* pBuffer = (TCHAR*)malloc(bufSizeTChar*sizeof(TCHAR));
	if (NULL == pBuffer)
	{
		return;
	}
	...
	// 在以下while循环中pBuffer指针将被移动
	TCHAR* pSrcBuffer = pBuffer;
	while(nCount)
	{
		TCHAR *pTemp =_tcsstr(pSrcBuffer,_T("&"));
		if(pTemp)
		{
			// we found &
			if(*(pTemp+1)==_T('&'))
			{
				...
				int nTempCount = DWORD(pTemp-pSrcBuffer)+1;
				ExtTextOut(hDC,pos.x,pos.y,ETO_CLIPPED,lpRect,pSrcBuffer,nTempCount,NULL);
				nCount -= nTempCount+1;
				pSrcBuffer = pTemp+2;
			}
			else
			{
				...
				int nTempCount = DWORD(pTemp-pSrcBuffer);
				ExtTextOut(hDC,pos.x,pos.y,ETO_CLIPPED,lpRect,pSrcBuffer,nTempCount,NULL);
				nCount -= nTempCount+1;
				pSrcBuffer = pTemp+1;
		...
		else
		{
			...
			ExtTextOut(hDC,pos.x,pos.y,ETO_CLIPPED,lpRect,pSrcBuffer,nCount,NULL);
			break;
		}
	}
...
CRect GetScreenRectFromWnd(HWND hWnd)
{
	CRect rect;
...
CRect GetScreenRectFromRect(LPCRECT pRect)
{
	CRect rect;
...
BOOL TrackPopupMenuSpecial(CMenu *pMenu, UINT uFlags, CRect rcTBItem, CWnd *pWndCommand, BOOL bOnSide)
{
	...
	if (nItemIndex!=UINT(-1) && GetMenuItemRect(pWnd->m_hWnd,pMenu->m_hMenu, nItemIndex, &itemRect))
	{
		...
		if(pMenu->GetMenuItemInfo(nItemIndex,&menuInfo,TRUE))
		{
			...
			if(menuInfo.fType&MF_OWNERDRAW)
			{
				...
				logFontMenu =  metrics.lfMenuFont;
#endif
...
CString CNewMenuItemData::GetString (HACCEL hAccel)
{
	...
	if (nEntries)
	{
		ACCEL *pAccel = (ACCEL *)malloc(nEntries*sizeof(ACCEL));
		if (NULL != pAccel)
		{
			if (::CopyAcceleratorTable (hAccel, pAccel, nEntries))
			{
				...
				for (int n = 0; n < nEntries; n++)
				{
					...
					if (pAccel [n].fVirt & FVIRTKEY)
					{
						...
						GetKeyNameText(vkey, keyname, sizeof(keyname)/sizeof(keyname[0]));
						sAccel += keyname;
					}
					else
					{
...
//lint --e{619} suppress "Suspicious cast"
...
BOOL CNewMenu::DestroyMenu()
{
	...
	m_hAccelToDraw = NULL;	
	m_pData = NULL;	//	赋值给menu的用户数据，可能外部已分配，CNewMenu将不负责释放
	m_hParentMenu = NULL;
	m_hTempOwner = NULL;
...
void CNewMenu::SetMenuBkColor(COLORREF clrMenuBk)
{
	m_clrMenuBk = clrMenuBk;
}
void CNewMenu::SetMenuBitmapBkColor(COLORREF clrBitmapBk)
{
	m_clrMenuBitmapBk = clrBitmapBk;
}
void CNewMenu::SetMenuBorderColor(COLORREF clrBorder)
{
	m_clrMenuBorderBk = clrBorder;
}
void CNewMenu::SetMenuItemSelColor(COLORREF clrSel)
{
	m_clrMenuItemSelBk = clrSel;
}
void CNewMenu::SetMenuItemSelTextColor(COLORREF clrSelText)
{
	m_clrMenuItemSelText = clrSelText;
}
void CNewMenu::SetMenuItemNorTextColor(COLORREF clrNormalText)
{
	m_clrMenuItemNorText = clrNormalText;
}
void CNewMenu::SetMenuItemDisableTextColor(COLORREF clrDisableText)
{
	m_clrMenuItemDisableText = clrDisableText;
}
// Jan-12-2005 - Mark P. Peterson - mpp@rhinosoft.com - http://www.RhinoSoft.com/
...
{
	ASSERT(m_pActMenuDrawing);
	if (NULL != m_pActMenuDrawing && NULL != lpMIS)
	{
...
{
	ASSERT(m_pActMenuDrawing);
	if (NULL != m_pActMenuDrawing && NULL != lpDIS)
	{
...
void CNewMenu::DrawItem_WinXP(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
	...
	COLORREF colorWindow = m_clrMenuBk;//RGB(55,65,77);//GetSysColor(COLOR_WINDOW);
	COLORREF colorMenuBar = bIsMenuBar?GetMenuBarColor(m_hMenu):colorWindow;
//	COLORREF colorMenuBar = GetMenuBarColor(m_hMenu);
	COLORREF colorMenu = MixedColor(colorWindow,colorMenuBar);
	COLORREF colorBitmap = m_clrMenuBitmapBk;//RGB(55,65,77);//MixedColor(GetMenuBarColor(m_hMenu),colorWindow);
	COLORREF colorSel = m_clrMenuItemSelBk;//RGB(54,92,149);//GetXpHighlightColor();
	COLORREF colorBorder = m_clrMenuBorderBk;//RGB(100,100,100);//GetSysColor(COLOR_HIGHLIGHT);//DarkenColor(128,colorMenuBar);
	...
	else
	{
		...
		if(!strText.IsEmpty())
		{
				...
				(bIsMenuBar && lpDIS->itemState&ODS_INACTIVE) )
			{
				...
				OldTextColor = pDC->SetTextColor(m_clrMenuItemDisableText);
			}
			else
			{
				...
				{
					OldTextColor = pDC->SetTextColor(m_clrMenuItemSelText);
				}
				else
				{
					OldTextColor = pDC->SetTextColor(m_clrMenuItemNorText/*GetSysColor(COLOR_MENUTEXT)*/);
				}
			}
		...
		{			
			...
			if(pMenuData->m_nMenuIconOffset<0 /*&& state&ODS_CHECKED */ && !checkflag)
			{
				...
				if(state&ODS_CHECKED || info.hbmpUnchecked)
				{
					...
					{
						pDC->FillSolidRect(rect, m_clrMenuBitmapBk);		//	draw checkbox background
						//pDC->FillSolidRect(rect,MixedColor(colorBitmap,GetSysColor(COLOR_HIGHLIGHT)));
					}
					CPen* pOldPen = pDC->SelectObject(&Pen);
					CBrush* pOldBrush = (CBrush*)pDC->SelectStockObject(HOLLOW_BRUSH);

					//pDC->Rectangle(rect);		//	draw checkbox border

					pDC->SelectObject(pOldBrush);
					pDC->SelectObject(pOldPen);
					if (state&ODS_CHECKED)
					{
						CRect rtInflateIcon(RectIcon);
						rtInflateIcon.InflateRect(2,((m_iconY-RectIcon.Height())>>1)+2);

						if (!info.hbmpChecked)
						{ // Checkmark
							DrawSpecialCharStyle(pDC,rtInflateIcon,98,state);
						}
						else if(!info.hbmpUnchecked)
						{ // Bullet
							DrawSpecialCharStyle(pDC,rtInflateIcon,105,state);
						}
						else
						{ // Draw Bitmap
							BITMAP myInfo = {0};
...
{
	int nLoc;
	CNewMenuItemData* pItemData = NULL;
...
CNewMenuItemData* CNewMenu::NewODMenu(UINT pos, UINT nFlags, UINT nID, LPCTSTR str)
{
	...
	{
		bModified = ModifyMenu(pos,nFlags,nID,(CBitmap*)str);
	}
...
BOOL CNewMenu::LoadToolBar(UINT n16ToolBarID, UINT n256BitmapID, COLORREF transparentColor, HMODULE hInst)
{
	...
	if (hGlobal)
	{
		...
		if (pData)
		{
			...
			// no need for delete
			WORD* pToolId = (WORD*)malloc(nSize);
			if (NULL != pToolId)
			{
...
CNewMenuItemData* CNewMenu::FindMenuItem(UINT nID)
{
	...
	{
		CNewMenuItemData* tmp = m_MenuItemList[i];
		if (tmp->m_nID == nID)
		{
			pData = tmp;
...
void CNewMenu::LoadCheckmarkBitmap(int unselect, int iSelect)
{
	if(unselect>0 && iSelect>0)
	{
		...
		BOOL flag2 = AddBitmapToImageList(m_checkmaps,iSelect);
...
BOOL CNewMenu::GetMenuText(UINT id, CString& str, UINT nFlags/*= MF_BYPOSITION*/)
{
	if(MF_BYPOSITION&nFlags)
	{
		...
		if(id<numMenuItems)
		{
			str = m_MenuItemList[id]->GetString(m_bDrawAccelerators ? m_hAccelToDraw : NULL);
			return TRUE;
		}
	}
	else
	{
		...
		if(NULL!=pMenu)
		{
			return pMenu->GetMenuText(uiLoc,str);
		}
	}
	return FALSE;
}
...
void CNewMenu::SynchronizeMenu()
{
	...
	for(j=0;j<GetMenuItemCount();++j)
	{
		...
		else if(state&MF_POPUP)
		{
			...
			GetMenuString(j,str,MF_BYPOSITION);

			if(!pItemData)
			{
				state &= ~(MF_USECHECKBITMAPS|MF_SEPARATOR);
				pItemData = NewODMenu(j,state|MF_BYPOSITION|MF_POPUP|MF_OWNERDRAW,submenu,str);
			}
			else if(!str.IsEmpty ())
			{
				pItemData->SetString(str);
		...
		else
		{
			...
			if(!pItemData)
			{
				pItemData = NewODMenu(j,state|MF_BYPOSITION|MF_OWNERDRAW,nID,str);
			}
			else
			{
				pItemData->m_nFlags = state|MF_BYPOSITION|MF_OWNERDRAW;
...
BOOL CNewMenu::SetMenuTitle(LPCTSTR pTitle, UINT nTitleFlags, CNewMenuIcons* pIcons, int nIndex)
{
	...
	if(numMenuItems)
	{
		...
		if(pMenuData->m_nFlags&MF_SEPARATOR)
		{
			...
			if(numMenuItems>1)
			{
				CNewMenuItemData* pMenuItemData = m_MenuItemList[1];

				if(nTitleFlags&MFT_SIDE_TITLE)
				{
					if(!(pMenuItemData->m_nFlags&MF_MENUBREAK))
					{
						pMenuItemData->m_nFlags |= MF_MENUBREAK;
						CMenu::ModifyMenu(1,MF_BYPOSITION|pMenuItemData->m_nFlags,pMenuItemData->m_nID,(LPCTSTR)pMenuItemData);
					}
				}
				else
				{
...
BOOL CNewMenu::SetMenuText(UINT id, CString str, UINT nFlags/*= MF_BYPOSITION*/ )
{
	if(MF_BYPOSITION&nFlags)
	{
		int numMenuItems = (int)m_MenuItemList.GetSize();
		if(id<UINT(numMenuItems))
		{
			...
			if(CMenu::ModifyMenu(id,MF_BYPOSITION|MF_STRING | nState, pItemData->m_nID, str))
			{
	...
	else
	{
		...
		if(NULL!=pMenu)
		{
			return pMenu->SetMenuText(uiLoc,str);
		}
	}
	return FALSE;
}
...
{
	clrBtnFace = RGB(255,255,255);
	clrBtnShadow = RGB(0,0,0);
	clrBtnHilite = RGB(192,192,192);
	clrBtnText = RGB(128,128,128);
	clrWindowFrame = RGB(0,0,0);
}
...
{
	CMenuTheme::UpdateSysColors();
	CMenuTheme::UpdateSysMetrics();
}
...
void CMenuTheme::GetBarColor(COLORREF &clrUpperColor, COLORREF &clrMediumColor, COLORREF &clrBottomColor, COLORREF &clrDarkLine)
{
	if(IsMenuThemeActive())
	{
		...
		// corrections from Andreas Sch鋜er
		switch(menuColor)
		{
...
BOOL CMenuTheme::OnEraseBkgnd(HWND hWnd, HDC hDC)
{
	...
	pDC->FillSolidRect(Rect,CNewMenu::m_clrMenuBk/*RGB(55,65,77)*/);//GetSysColor(COLOR_MENU));
...
BOOL CMenuThemeXP::OnDrawBorder(HWND hWnd, HDC hDC, BOOL bOnlyBorder)
{
	...
	if(DoDrawBorder() && pData)
	{
		...
		COLORREF crMenuBar = CNewMenu::m_clrMenuBk;//CNewMenu::GetMenuColor(pData->m_hMenu);
		COLORREF crWindow = CNewMenu::m_clrMenuBk;//GetSysColor(COLOR_WINDOW);
		COLORREF crThinBorder = CNewMenu::m_clrMenuBorderBk;//CNewMenu::m_clrMenuBorderBk; //MixedColor(crWindow,crMenuBar);
		COLORREF clrBorder = CNewMenu::m_clrMenuBorderBk; 		
...
BOOL CMenuThemeXP::OnEraseBkgnd(HWND hWnd, HDC hDC)
{
	...
	pDC->FillSolidRect (Rect,CNewMenu::m_clrMenuBk/*CNewMenu::GetMenuBarColor(pData->m_hMenu)*/);
...
//lint -sem(CTypedPtrMap::SetAt, custodial(2))
				...
				if(hDC)
				{
					if(CNewMenu::m_pActMenuDrawing->OnDrawBorder(hWnd,hDC))
					{
						...
						if(CNewMenu::m_pActMenuDrawing->OnCalcFrameRect(hWnd,rect))
						{
							...
							{
								result = DefWindowProc(hWnd, uMsg, (WPARAM)rgn.m_hObject, lParam);						
							}
	...
	case WM_PRINT:
		if(pData && pData->m_bDoSubclass)
		{
			if(CNewMenu::m_pActMenuDrawing->DoDrawBorder())
			{
				...
				// new
				//dc.FillSolidRect(0,0,rect.Width(), rect.Height(),RGB(55,65,77));
	...
	if(pData==NULL)
	{
		...
		if (oldWndProc != NULL && GetProp(hWnd, _OldMenuProc) == NULL)
		{
			...
			if ((WNDPROC)GetProp(hWnd, _OldMenuProc) == oldWndProc)
			{
				...
				CMenuHookData* pHookData=GetMenuHookData(hWnd);
				ASSERT(pHookData==NULL);
				if(pHookData==NULL)
				{
					pHookData = new CMenuHookData(hWnd,bSpecialWnd);
					m_MenuHookData.SetAt (hWnd,pHookData);
...
//lint --e{578} suppress "Declaration of symbol 'messageMap' hides symbol 'CNewFrame<CDialog>::messageMap"
	...
	if (!bSysMenu && NULL != pMenu)
	{
		...
		else if ((hParentMenu = ::GetMenu(m_hWnd)) != NULL)
		{
				...
				(hParentMenu = ::GetMenu(pParent->m_hWnd)) != NULL)
			{
				...
				for (int nItem = 0; nItem < nIndexMax; nItem++)
				{
					if (::GetSubMenu(hParentMenu, nItem) == pMenu->m_hMenu)
					{
...
//lint --e{578} suppress "Declaration of symbol 'messageMap' hides symbol 'CNewFrame<CMiniDockFrameWnd>::messageMap'"
...
//lint --e{578} suppress "Declaration of symbol '_messageEntries' hides symbol 'CNewFrame<CMDIFrameWnd>::_messageEntries'"
