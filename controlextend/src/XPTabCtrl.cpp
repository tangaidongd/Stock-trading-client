...
{	
	//lint --e{529} suppress " Symbol 'nHeight' (line 208) not subsequently referenced, pclint 好像对汇编不起作用，所以屏蔽"
	...
	// if bottom oriented flip the body contest only (no shadows were flipped)
...
{
	//lint -e{522} suppress "期望有void类型、赋值、++、――"
    UNUSED_ALWAYS(pNMHDR);
	m_ixSelOld=GetCurSel();
	*pResult=0L;
}

// zhangbo 0313 #// below, to notify the parent window "select change event"
//----------------------------------------------------------------------------------------------------------
