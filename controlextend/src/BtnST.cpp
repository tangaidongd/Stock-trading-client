...
//lint -esym(547, BS_TYPEMASK) suppress "Redefinition of symbol 'BS_TYPEMASK' conflicts with line 9292"
#define BS_TYPEMASK SS_TYPEMASK

CButtonSTMain::CButtonSTMain()
{
	m_bIsPressed		= FALSE;
	m_bIsFocused		= FALSE;
	m_bIsDisabled		= FALSE;
	m_bMouseOnButton	= FALSE;
	m_bmpBK4.m_hObject = NULL;
	m_bmpBK2.m_hObject = NULL;
	m_nStateCount = 0;
	...
	m_bShowText = FALSE;
	m_bIsOffset = TRUE;	// lcq add 20131210
} // End of CButtonSTMain
...
BEGIN_MESSAGE_MAP(CButtonSTMain, CButton)
    //{{AFX_MSG_MAP(CButtonSTMain)
	ON_WM_SETCURSOR()
	ON_WM_KILLFOCUS()
	ON_WM_MOUSEMOVE()
...
void CButtonSTMain::FreeResources(BOOL bCheckForNULL)
{
	if (bCheckForNULL)
	{
		// Destroy icons
...
void CButtonSTMain::PreSubclassWindow() 
{
	UINT nBS;
...
UINT CButtonSTMain::OnGetDlgCode() 
{
...
BOOL CButtonSTMain::PreTranslateMessage(MSG* pMsg) 
{
	InitToolTip();
...
LRESULT CButtonSTMain::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
...
HBRUSH CButtonSTMain::CtlColor(CDC* pDC, UINT nCtlColor) 
{
	return (HBRUSH)::GetStockObject(NULL_BRUSH); 
} // End of CtlColor

void CButtonSTMain::OnSysColorChange() 
{
...
LRESULT CButtonSTMain::OnSetStyle(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CButtonSTMain::OnSetCheck(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CButtonSTMain::OnGetCheck(WPARAM wParam, LPARAM lParam)
{
	ASSERT(m_bIsCheckBox);
	return GetCheck();
} // End of OnGetCheck
...
#ifdef	BTNST_USE_BCMENU
LRESULT CButtonSTMain::OnMenuChar(UINT nChar, UINT nFlags, CMenu* pMenu) 
{
	LRESULT lResult;
...
#ifdef	BTNST_USE_BCMENU
void CButtonSTMain::OnMeasureItem(int nIDCtl, LPMEASUREITEMSTRUCT lpMeasureItemStruct) 
{
	BOOL bSetFlag = FALSE;
...
void CButtonSTMain::OnEnable(BOOL bEnable) 
{
...
void CButtonSTMain::OnKillFocus(CWnd * pNewWnd)
{
	CButton::OnKillFocus(pNewWnd);
	CancelHover();
} // End of OnKillFocus

void CButtonSTMain::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized) 
{
	CButton::OnActivate(nState, pWndOther, bMinimized);
	if (nState == WA_INACTIVE)	CancelHover();
} // End of OnActivate

void CButtonSTMain::OnCancelMode() 
{
	CButton::OnCancelMode();
	CancelHover();
} // End of OnCancelMode

BOOL CButtonSTMain::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
...
void CButtonSTMain::CancelHover()
{
...
void CButtonSTMain::OnMouseMove(UINT nFlags, CPoint point)
{
...
LRESULT CButtonSTMain::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
	CancelHover();
	return 0;
} // End of OnMouseLeave

BOOL CButtonSTMain::OnClicked() 
{	
	SetFocus();
...
void CButtonSTMain::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	if (m_bmpBK4.m_hObject)
	{
		DrawBkBitmap4(pDC);
	}
	if (m_bmpBK2.m_hObject)
	{
		DrawBkBitmap2(pDC);
	}
	...
	if (sTitle.IsEmpty() == FALSE)
	{
		...
		if (m_bIsPressed && m_bIsCheckBox == FALSE&& m_bIsOffset)
...
void CButtonSTMain::PaintBk(CDC* pDC)
{
...
HBITMAP CButtonSTMain::CreateBitmapMask(HBITMAP hSourceBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTransColor)
{
	HBITMAP		hMask		= NULL;
	HDC			hdcSrc		= NULL;
	HDC			hdcDest		= NULL;
...
//
void CButtonSTMain::PrepareImageRect(BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, DWORD dwWidth, DWORD dwHeight, CRect* rpImage)
{
	CRect rBtn;
	...
	if (bIsPressed && m_bIsCheckBox == FALSE && m_bIsOffset)	// lcq add 20131210
		rpImage->OffsetRect(1, 1);
} // End of PrepareImageRect

void CButtonSTMain::DrawTheIcon(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, BOOL bIsDisabled)
{
	BYTE		byIndex		= 0;
...
void CButtonSTMain::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rItem, CRect *rCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	HDC			hdcBmpMem	= NULL;
...
void CButtonSTMain::DrawBkBitmap4(CDC *pDC)
{
	if (!m_bmpBK4.m_hObject)
		return;
	...
	HGDIOBJ hOldBmp = memDC.SelectObject(m_bmpBK4.m_hObject);
	int nHeight = m_bmpInfo.bmHeight/m_nStateCount;
	if (m_bIsDisabled)
	{
		pDC->StretchBlt(0,0,crClient.Width(),crClient.Height(),&memDC,0,0,m_bmpInfo.bmWidth,nHeight,SRCCOPY);
		return;
	}
	// 如果鼠标经过
	if (m_bMouseOnButton)
	{
		...
		if (m_bIsPressed)
		{
			pDC->StretchBlt(0,0,crClient.Width(),crClient.Height(),&memDC,0,nHeight*2,m_bmpInfo.bmWidth,nHeight,SRCCOPY);
		}
		else
		{
			pDC->StretchBlt(0,0,crClient.Width(),crClient.Height(),&memDC,0,nHeight*3,m_bmpInfo.bmWidth,nHeight,SRCCOPY);
		}
	}
	else
	{
		pDC->StretchBlt(0,0,crClient.Width(),crClient.Height(),&memDC,0,nHeight*1,m_bmpInfo.bmWidth,nHeight,SRCCOPY);
	}
	memDC.SelectObject(hOldBmp);
}

void CButtonSTMain::DrawBkBitmap2(CDC *pDC)
{
	if (!m_bmpBK2.m_hObject)
		return;
	...
	CDC memDC;
	memDC.CreateCompatibleDC(pDC);
	HGDIOBJ hOldBmp = memDC.SelectObject(m_bmpBK2.m_hObject);
	int nHeight = m_bmpInfo.bmHeight/m_nStateCount;
	// 如果鼠标经过
	if (m_bMouseOnButton)
	{
...
//
HICON CButtonSTMain::CreateGrayscaleIcon(HICON hIcon)
{
...
//
DWORD CButtonSTMain::SetIcon(int nIconIn, int nIconOut)
{
	HICON		hIconIn			= NULL;
	HICON		hIconOut		= NULL;
...
//			Failed loading the specified resource.
//
DWORD CButtonSTMain::SetIcon(HICON hIconIn, HICON hIconOut)
{
	BOOL		bRetValue;
	ICONINFO	ii;
...
//
DWORD CButtonSTMain::SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut)
{
...
BOOL  CButtonSTMain::SetBkBitmap(int nResourceID,int nStateCount)
{
	BOOL bRet = FALSE;
	if (4 == nStateCount)
	{
		...
		bRet = m_bmpBK4.LoadBitmap(nResourceID);
		if (bRet)
		{
			m_bmpBK4.GetBitmap(&m_bmpInfo);
		}
		
	}
	else if (2 == nStateCount)
	{
		if (m_bmpBK2.m_hObject)
			m_bmpBK2.Detach();
		m_nStateCount = nStateCount;
		bRet = m_bmpBK2.LoadBitmap(nResourceID);
		if (bRet)
		{
			m_bmpBK2.GetBitmap(&m_bmpInfo);
		}
	}
	return bRet;
}
...
//			Failed creating mask bitmap.
//
DWORD CButtonSTMain::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut)
{
	int		nRetValue;
	BITMAP	csBitmapSize;
...
//				By default, CButtonSTMain buttons are flat.
//		[IN]	bRepaint
...
//			Function executed successfully.
//
DWORD CButtonSTMain::SetFlat(BOOL bFlat, BOOL bRepaint)
{
	m_bIsFlat = bFlat;
...
//				By default, CButtonSTMain buttons have ST_ALIGN_HORIZ alignment.
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
//
// Return value:
...
//
DWORD CButtonSTMain::SetAlign(BYTE byAlign, BOOL bRepaint)
{
	switch (byAlign)
	{    
		case ST_ALIGN_HORIZ:
...
//			Function executed successfully.
//
DWORD CButtonSTMain::SetCheck(int nCheck, BOOL bRepaint)
{
	if (m_bIsCheckBox)
	{
...
//			Function executed successfully.
//
DWORD CButtonSTMain::SetDefaultColors(BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CButtonSTMain::SetColor(BYTE byColorIndex, COLORREF crColor, BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CButtonSTMain::GetColor(BYTE byColorIndex, COLORREF* crpColor)
{
	if (byColorIndex >= BTNST_MAX_COLORS)	return BTNST_INVALIDINDEX;
...
//
DWORD CButtonSTMain::OffsetColor(BYTE byColorIndex, short shOffset, BOOL bRepaint)
{
	BYTE	byRed = 0;
	BYTE	byGreen = 0;
	BYTE	byBlue = 0;
...
//			Function executed successfully.
//
DWORD CButtonSTMain::SetAlwaysTrack(BOOL bAlwaysTrack)
{
...
//			Failed loading the specified resource.
//
DWORD CButtonSTMain::SetBtnCursor(int nCursorId, BOOL bRepaint)
{
	HINSTANCE	hInstResource = NULL;
...
//			Function executed successfully.
//
DWORD CButtonSTMain::DrawBorder(BOOL bDrawBorder, BOOL bRepaint)
{
...
//			Function executed successfully.
//
DWORD CButtonSTMain::DrawFlatFocus(BOOL bDrawFlatFocus, BOOL bRepaint)
{
...
void CButtonSTMain::InitToolTip()
{
...
//
void CButtonSTMain::SetTooltipText(int nText, BOOL bActivate)
{
	CString sText;

	// Load string resource
	
	if (sText.LoadString(nText))
	{
...
//				If TRUE the tooltip will be created active.
//
void CButtonSTMain::SetTooltipText(LPCTSTR lpszText, BOOL bActivate)
{
...
//
void CButtonSTMain::ActivateTooltip(BOOL bActivate)
{
...
//				If TRUE the control will be repainted.
//
void CButtonSTMain::DrawTransparent(BOOL bRepaint)
{
...
//			Function executed successfully.
//
DWORD CButtonSTMain::SetURL(LPCTSTR lpszURL)
{
...
//
#ifndef	BTNST_USE_BCMENU
DWORD CButtonSTMain::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bRepaint)
{
	HINSTANCE	hInstResource	= NULL;
...
//
#ifdef	BTNST_USE_BCMENU
DWORD CButtonSTMain::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bWinXPStyle, UINT nToolbarID, CSize sizeToolbarIcon, COLORREF crToolbarBk, BOOL bRepaint)
{
	BOOL	bRetValue = FALSE;
...
// This is a virtual function that can be rewritten in CButtonSTMain-derived classes
...
//			Function executed successfully.
//
DWORD CButtonSTMain::OnDrawBackground(CDC* pDC, LPCRECT pRect)
{
	COLORREF	crColor;
...
//			Function executed successfully.
//
DWORD CButtonSTMain::OnDrawBorder(CDC* pDC, LPCRECT pRect)
{
	if (m_bIsPressed)
...
void CButtonSTMain::SetShowText(BOOL bShow)
{
  m_bShowText = bShow;
  Invalidate();
} // End of SetShowText

void CButtonSTMain::SetEnable(BOOL bEnabled)
{
