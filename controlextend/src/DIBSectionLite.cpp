...
CDIBSectionLite::CDIBSectionLite()
{
	...
	CDIBSectionLite::DeleteObject(); // This will initialise to a known state - ie. empty
}

CDIBSectionLite::~CDIBSectionLite()
{
    CDIBSectionLite::DeleteObject();
}
...
#endif	
    memset((void*)&m_DIBinfo, 0, sizeof(m_DIBinfo)); 
...
void CDIBSectionLite::AssertValid() const
{
	...
	if (NULL != m_hBitmap)
	{
		...
		ASSERT(m_iColorTableSize <= 256);

	}
   
	//	目前只能这种处理，外部调用可能未对对象进行有效性判断
	CObject::AssertValid();
}
...
{ 
   ...
   // We will only be able to get a HDRAWDIB if dithering is enabled
    ...
    else
    {
        ...
        const CDC* pMemDC = GetMemoryDC(pDC, FALSE);
        if (!pMemDC)
            return FALSE;
        ...
        bResult = pDC->BitBlt(ptDest.x, ptDest.y, size.cx, size.cy, (CDC*)pMemDC, 
...
BOOL CDIBSectionLite::SetBitmapExt(LPBITMAPINFO lpBitmapInfo, LPVOID lpBits)
{
    DeleteObject();
    ...
    TRY {
        ...
        memcpy((void*)&m_DIBinfo, lpBitmapInfo, dwBitmapInfoSize);
...
BOOL CDIBSectionLite::Load(LPCTSTR lpszFileName)
{
    ...
    if (!SetBitmapExt((LPBITMAPINFO) BmpInfo, pBits))
    {
...
BOOL CDIBSectionLite::Copy(CDIBSectionLite& bmpLite) 
{ 
    BOOL bResult = SetBitmapExt(bmpLite.GetBitmapInfo(), bmpLite.GetDIBits()); 

    if (bResult)
    {
        SetColorTable(bmpLite.GetColorTableSize(), bmpLite.GetColorTable());
#ifndef DIBSECTION_NO_PALETTE
        CreatePalette();
#endif
#ifndef DIBSECTION_NO_DITHER
        SetDither(bmpLite.GetDither());
#endif
    }
...
const CDC* CDIBSectionLite::GetMemoryDC(CDC* pDC /*=NULL*/, BOOL bSelectPalette /*=TRUE*/)
{
#ifdef DIBSECTION_NO_MEMDC_REUSE
    ReleaseMemoryDC(TRUE);
#else
...
BYTE CDIBSectionLite::GetPixelIndex(long x,long y)
{
	if( !IsValid() )
		return 0 ;
	...
	DWORD dwEffWidth = BytesPerLine( width, nBitCount );
	...
	if ( nBitCount == 8 )
	{
		LPBYTE pBits = (LPBYTE)GetDIBits();
		ASSERT( pBits );
		if (NULL == pBits)
		{
			return 0;
		}
		return pBits[y * dwEffWidth + x];
	} 
	else 
	{
		LPBYTE pBits = (LPBYTE)GetDIBits();
		...
		BYTE pos;
		BYTE iDst = pBits[y * dwEffWidth + (x * nBitCount >> 3)];
		if ( nBitCount == 4 )
		{
			pos = (BYTE)(4*(1-x%2));
			iDst &= (0x0F<<pos);
...
RGBQUAD CDIBSectionLite::GetPixelColor(long x,long y)
{
	RGBQUAD rgb ;
	rgb.rgbBlue = 0;
	rgb.rgbGreen = 0;
	...
	LPBYTE pBits = (LPBYTE)GetDIBits();
	ASSERT( pBits );
	int nColors = NumColorEntries( nBitCount );
	int iDepth = 3 ;
	if( nBitCount == 24 )
		iDepth = 3 ;
	...
	if ( nColors ) 
	{
		return GetPaletteColor(GetPixelIndex(x,y));
	}
	else 
	{
		BYTE* iDst = pBits + y * dwEffWidth + x * iDepth;
		//rgb.rgbRed = iDst[0];
		//rgb.rgbGreen = iDst[1];
		//rgb.rgbBlue = iDst[2];
		rgb.rgbBlue = *iDst++;
...
void CDIBSectionLite::SetPixelIndex(long x,long y,BYTE i)
{
	if( !IsValid() )
		return ;
	...
	if ( ( pBits == NULL ) || ( nColors ==0 )||
		( x < 0 ) || ( y < 0) || ( x >= width ) || ( y >= height)) 
		return ;

	if ( nBitCount == 8 ) 
	{
		pBits[ y * dwEffWidth + x ] = i;
		return;
	} 
	else
	{
...
RGBQUAD CDIBSectionLite::GetPaletteColor(BYTE idx)
{
	RGBQUAD rgb = {0,0,0,0};
...
void CDIBSectionLite::SetPixelColor(long x,long y,COLORREF cr)
{
	SetPixelColor(x,y,RGBtoRGBQUAD(cr));
}


void CDIBSectionLite::SetPixelColor(long x,long y,RGBQUAD c)
{
	if( !IsValid() )
		return  ;
...
BYTE CDIBSectionLite::GetNearestIndex(RGBQUAD c)
{
	if( !IsValid() )
		return 0 ;
...
RGBQUAD CDIBSectionLite::RGBtoRGBQUAD(COLORREF cr)
{
	RGBQUAD c;
...
COLORREF CDIBSectionLite::RGBQUADtoRGB (RGBQUAD c)
{
	return RGB(c.rgbRed,c.rgbGreen,c.rgbBlue);
}

BOOL CDIBSectionLite::IsValid ( )
{
	return m_ppvBits != NULL ;
}

BOOL CDIBSectionLite::CompoundDIB(int left,int top,CDIBSectionLite *pDestDib,int nAlpha)
{
    if(!IsValid()||pDestDib==NULL) 
		return FALSE;

	LPBYTE lpvBuf=NULL; // 目标图象数据指针(背景)
    LPBYTE lpvBufSrc=NULL; // 源图数据指针(前景)
   


    // 源图象信息 // 
	int nSrcBitCount = GetBitCount() ;
    lpvBufSrc = (LPBYTE)GetDIBits(); 
    int cxSrc = GetWidth(); // 源图象宽度
    int cySrc = GetHeight(); // 源图象高度
    // 计算图象每行的字节数(图象位数 x 图象宽度,如果不能被2整除则在每行后面添加一个0字节)
    int nBytesPerLineSrc = BytesPerLine( cxSrc , nSrcBitCount );  

    // 目标图象信息 //
	int nDestBitCount = pDestDib->GetBitCount();
    lpvBuf = (LPBYTE)(pDestDib->GetDIBits());
    int cx = pDestDib->GetWidth();
    int cy = pDestDib->GetHeight();
    int nBytesPerLine = pDestDib->BytesPerLine( cx , nDestBitCount );

	if( cx < cxSrc || cy < cySrc )
		return FALSE;
	...
	if( nDestBitCount == 24 &&  nSrcBitCount == 24 )
	{
		for ( int y= cySrc; y>0 && (cy-top-cySrc+y)>0;y-- )
		{
			lpbPnt=(LPBYTE)lpvBuf+nBytesPerLine*(cy-top-cySrc+y-1)+left*3;
			lpbPntSrc=(LPBYTE)lpvBufSrc+nBytesPerLineSrc*(y-1);
			for ( int x=0; x<cxSrc&&x<(cx-left); x++ )
			{
				for ( int i=0 ;i<3 ;i++ )
				{
					*lpbPnt=(*lpbPnt*(255-nAlpha)+*(lpbPntSrc)*nAlpha)/255;
					++lpbPntSrc;
					++lpbPnt;
				}
	...
	else if( nDestBitCount == 32 &&  nSrcBitCount == 32 )
	{
		for ( int y=cySrc; y>0&&(cy-top-cySrc+y)>0;y-- )
		{
			lpbPnt=(LPBYTE)lpvBuf+nBytesPerLine*(cy-top-cySrc+y-1)+left*4;
			lpbPntSrc=(LPBYTE)lpvBufSrc+nBytesPerLineSrc*(y-1);
			for ( int x=0; x<cxSrc&&x<(cx-left); x++ )
			{
				for ( int i=0 ;i<3 ;i++ )
	...
	else if( nDestBitCount == 32 && nSrcBitCount == 24 )
	{
		for ( int y=cySrc; y>0&&(cy-top-cySrc+y)>0;y-- )
		{
			lpbPnt=(LPBYTE)lpvBuf+nBytesPerLine*(cy-top-cySrc+y-1)+left*4;
	...
	else if( nDestBitCount ==24 && nSrcBitCount == 32 )
	{
		for ( int y=cySrc; y>0&&(cy-top-cySrc+y)>0;y-- )
		{
			lpbPnt=(LPBYTE)lpvBuf+nBytesPerLine*(cy-top-cySrc+y-1)+left*3;
...
BOOL    CDIBSectionLite::MarkColor(const COLORREF &clr)
{
	if( !IsValid() )
		return FALSE;
...
COLORREF  CDIBSectionLite::GetMarkColor(const COLORREF clrOld,const COLORREF &clr,int nAlpha)
{
	return (clr*(255-nAlpha)+clrOld*nAlpha)/255;
}

BOOL CDIBSectionLite::CompoundDIB(int left,int top,CDIBSectionLite *pDib,CDIBSectionLite *pDIBAlpha)
{
    if(!IsValid()||pDib==NULL||pDIBAlpha==NULL) 
		return false;
	
	LPBYTE lpvBuf=NULL; // 目标图象数据指针(背景)
    LPBYTE lpvBufSrc=NULL; // 源图数据指针(前景)
    LPBYTE lpvBufAlpha=NULL; // Alpha通道数据指针
    ...
    // 目标图象信息 //
	int nDestBitCount = pDib->GetBitCount();
    lpvBuf = (LPBYTE)(pDib->GetDIBits());
    int cx = pDib->GetWidth();
    int cy = pDib->GetHeight();
    int nBytesPerLine = pDib->BytesPerLine( cx , nDestBitCount );

    // Alpha通道信息 //
    lpvBufAlpha = (LPBYTE)(pDIBAlpha->GetDIBits());
	int alphawidth = pDIBAlpha->GetWidth();
	int alphaheight = pDIBAlpha->GetHeight();
    int nBytesPerLineAlpha = pDIBAlpha->BytesPerLine( alphawidth ,pDIBAlpha->GetBitCount() );

	if( cx < cxSrc || cy < cySrc )
		return FALSE;

	if( alphawidth < cxSrc|| alphaheight < cySrc )
		return FALSE;
	...
	if( nDestBitCount == 24 &&  nSrcBitCount == 24 )
	{
		for ( int y= cySrc; y>0 && (cy-top-cySrc+y)>0;y-- )
		{
			...
			{
				int alpha = 0;
				if( pDIBAlpha->GetBitCount() == 8 )
				{
					alpha=*lpbPntAlpha;
					++lpbPntAlpha;
				}
				
				else if( pDIBAlpha->GetBitCount() == 24 )
				{
					alpha=*lpbPntAlpha;
					lpbPntAlpha+=3;
				}
				else if( pDIBAlpha->GetBitCount() == 32 )
				{
					alpha=*lpbPntAlpha;
					lpbPntAlpha+=4;
				}
				...
				for ( int i=0 ;i<3 ;i++ )
				{
					*lpbPnt= ((*lpbPnt)*alpha+*(lpbPntSrc)*(255-alpha))/255;
					++lpbPnt;
					++lpbPntSrc;
				}
			}
...
//变成灰度图
BOOL CDIBSectionLite::MakeGray( BOOL bSetPalette )
{
	if( !IsValid() )  
		return( FALSE );

	int nBitsPerPixel = GetBitCount ();
	int nWidth = GetWidth();
	

	unsigned char  *pBits = NULL, *pTemp  = NULL;
	BITMAPINFOHEADER *pBIH = NULL;
	RGBQUAD *pPalette = NULL;
	int nWidthBytes = 0, nNumColors = 0, x = 0, y = 0, i = 0;
	WORD wColorBits = 0, *pColorBits = NULL;

	nWidthBytes =  BytesPerLine( nWidth , nBitsPerPixel );
	pBIH = (BITMAPINFOHEADER *) GetBitmapInfoHeader();
	ASSERT( pBIH );
	if (NULL == pBIH)
	{
		return FALSE;
	}
	nNumColors = NumColorEntries( nBitsPerPixel );
	pPalette = (RGBQUAD *)GetColorTable();
	ASSERT( pPalette );
	if( pPalette == NULL )
		return FALSE;
	pBits = (unsigned char *)GetDIBits();
	ASSERT( pBits );
	if( pBits == NULL )
		return FALSE;
	...
	switch( nBitsPerPixel )
	{
		...
		case 4:
			{
				unsigned char *pLookup = new unsigned char [16];
		...
		case 8:
			{
				unsigned char* pLookup = new unsigned char [256];
				if( pLookup == NULL ) break;
...
BOOL CDIBSectionLite::Rotate(CDIBSectionLite *pImagData , float angle )
{
	if ( !pImagData->IsValid() ) 
		return FALSE;
	...
	double ang = -angle*acos((double)0)/90;
	int newWidth, newHeight;
	int nWidth = pImagData->GetWidth();
	int nHeight= pImagData->GetHeight();
	...
	CDIBSectionLite imgDest;
	
	imgDest.Create(newWidth,newHeight,pImagData->GetBitCount() );
	int nColor = imgDest.NumColorEntries( imgDest.GetBitCount() );
	if( nColor > 0 )
	{
		RGBQUAD *SrcTable = (RGBQUAD *)pImagData->GetColorTable();
		ASSERT( SrcTable );
		RGBQUAD *pDestTable = imgDest.GetColorTable();
		ASSERT( pDestTable );
		CopyMemory( pDestTable , SrcTable , sizeof( RGBQUAD ) * nColor );
	}
	...
	{ //RGB
		for (y = leftTop.y, newY = 0; y<=leftBottom.y; y++,newY++)
		{
			for (x = leftTop.x, newX = 0; x<=rightTop.x; x++,newX++)
			{
				...
				imgDest.SetPixelColor(newX,newY,pImagData->GetPixelColor(oldX,oldY));
			}
		}
	} else { //PALETTE
		for (y = leftTop.y, newY = 0; y<=leftBottom.y; y++,newY++)
		{
			for (x = leftTop.x, newX = 0; x<=rightTop.x; x++,newX++)
			{
				oldX = (long)(x*cos_angle + y*sin_angle - 0.5);
				oldY = (long)(y*cos_angle - x*sin_angle - 0.5);
				imgDest.SetPixelIndex(newX,newY,pImagData->GetPixelIndex(oldX,oldY));
			}
		}
	}
	
	//select the destination
	pImagData->Copy( imgDest);
	
	return true;
}

BOOL CDIBSectionLite::Create(DWORD dwWidth, DWORD dwHeight, long wBpp, long imagetype)
{
	...
	DIBINFO	BmpInfo;
	BITMAPINFOHEADER *pBitHeadinfo = &BmpInfo.bmiHeader;
	ASSERT( pBitHeadinfo );
	// set the correct bpp value
    switch (wBpp)
	{
	case 1:
		pBitHeadinfo->biClrUsed = 2;	break;
	case 2:
		pBitHeadinfo->biClrUsed = 2;	break;
	case 4:
		pBitHeadinfo->biClrUsed = 16; break;
	case 8:
		pBitHeadinfo->biClrUsed = 256; break;
	default:
		pBitHeadinfo->biClrUsed = 0;
    }
	
	//set the common image informations
	//m_bColorType = (BYTE)((wBpp>8) ? COLORTYPE_COLOR : COLORTYPE_PALETTE);
    DWORD	dwEffWidth = ((((wBpp * dwWidth) + 31) / 32) * 4);
    //m_dwType = imagetype;
	
    // initialize BITMAPINFOHEADER
	pBitHeadinfo->biSize = sizeof( BITMAPINFOHEADER );
    pBitHeadinfo->biWidth = dwWidth;		// fill in width from parameter
    pBitHeadinfo->biHeight = dwHeight;	// fill in height from parameter
    pBitHeadinfo->biPlanes = 1;			// must be 1
    pBitHeadinfo->biBitCount = (WORD)wBpp;		// from parameter
    pBitHeadinfo->biCompression = BI_RGB;    
    pBitHeadinfo->biSizeImage = dwEffWidth * dwHeight;
	//    head.biXPelsPerMeter = 0; See SetXDPI
	//    head.biYPelsPerMeter = 0; See SetYDPI
	pBitHeadinfo->biXPelsPerMeter = 0;
	pBitHeadinfo->biYPelsPerMeter = 0;
	
	int nPaletteSize = NumColorEntries( wBpp );
	
	//pBitHeadinfo->biClrUsed = nPaletteSize;
	pBitHeadinfo->biClrImportant = nPaletteSize;
	
	SetBitmapExt( BmpInfo,NULL);

    return TRUE; 
}

Status CDIBSectionLite::DrawImageGp(CDC *pDC, Image *pImage, CRect rcShow)
{
	ASSERT( pDC );
	ASSERT( pImage );
	if( !pDC || !pImage)
	...
	Rect RectShow(rcShow.left,rcShow.top,rcShow.Width(),rcShow.Height() );
	...
	graphics.DrawImage(pImage,RectShow);

	return graphics.GetLastStatus();
}


Status CDIBSectionLite::DrawImageGp(CDC *pDC, Image* pImage, int x,int y,int nsrcx,int nsrcy,int nsrcw,int nsrch)
{
	ASSERT( pDC );
	ASSERT( pImage );
	if( !pDC || !pImage)
	...
	graphics.DrawImage(pImage,x,y,nsrcx,nsrcy,nsrcw,nsrch,UnitPixel);
	
	return graphics.GetLastStatus();
}

Status CDIBSectionLite::DrawImageGp(CDC *pDC, Image* pImage, CRect rcDes,int nsrcx,int nsrcy,int nsrcw,int nsrch)
{
	ASSERT( pDC );
	ASSERT( pImage );
	if( !pDC || !pImage)
	...
	Rect rc(rcDes.left,rcDes.top,rcDes.Width(),rcDes.Height());
	graphics.DrawImage(pImage,rc,nsrcx,nsrcy,nsrcw,nsrch,UnitPixel);
	
	return graphics.GetLastStatus();
}

Status  CDIBSectionLite::DrawImageGp(Graphics* g, Image* pImage, Rect *pRect, 
								GraphicsPath* pPath,ImageAttributes *pIab)
{
	ASSERT( g );
	ASSERT( pImage );
	ASSERT( pRect );
	...
	if( !pIab )
	{
		...
		g->SetClip(&OldRegion,CombineModeReplace);
	...
	else
	{
		...
		g->DrawImage( pImage, *pRect ,0, 0, iWidth,iHeight, 
			UnitPixel, pIab);
...
void CDIBSectionLite::TransparentBlt( CDC *pDC,      // 目标DC 
...
{ 
	...
	::TransparentBlt(hImageDC.GetSafeHdc(),0, 0, nWidthDest, nHeightDest, 
		hdcSrc->GetSafeHdc(), nXOriginSrc, nYOriginSrc, nWidthSrc, nHeightSrc, SRCCOPY); 
	// 设置透明色 
...
void CDIBSectionLite::DrawBlt(CDC* pDC, int nX, int nY, 
						 int nWidth,int nHeight, int nXSrc, int nYSrc,DWORD dwRop,CBitmap *bit)
{
    CBitmap *pBitmap = bit;
    ...
    pDC->BitBlt(nX, nY, nWidth, nHeight, &dcMem, nXSrc, nYSrc, dwRop);
    dcMem.SelectObject(pOldBitmap);
	
	dcMem.DeleteDC();
}

CBitmap *CDIBSectionLite::GeBitmap()
{
	if( !IsValid())
		return NULL;
...
void CDIBSectionLite::DrawStretchDIB(CDC* pDC, int nX, int nY, 
					int nWidth,int nHeight, int nXSrc, int nYSrc,
						 int nWidthSrc,int nHeightSrc,DWORD dwRop,CBitmap *bit)
{
	CBitmap *pBitmap = bit;
    ...
    CBitmap* pOldBitmap = dcMem.SelectObject(pBitmap);
    pDC->StretchBlt(nX, nY, nWidth, nHeight, &dcMem, nXSrc, nYSrc,nWidthSrc, nHeightSrc,dwRop);
    dcMem.SelectObject(pOldBitmap);
