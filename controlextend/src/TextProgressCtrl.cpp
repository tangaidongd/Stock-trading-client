...
void CTextProgressCtrl::OnPaint() 
{
	...
	int iStep = m_nMax - m_nMin;
	if (iStep != 0)
		Fraction = (double)(m_nPos - m_nMin) / ((double)(iStep));
    ...
    if (m_bShowText)
    {
		...
		if ( NULL != m_FontText.GetSafeHandle() )
		{
        ...
        else
		{
			...
			str.Format(L"%d/%d", m_nPos, m_nMax);
		}
...
void CTextProgressCtrl::SetTextFont(CFont* pFont)
{
	ASSERT(NULL != pFont);
	...
	if(NULL != pFont)
	{
		LOGFONT lg;
		pFont->GetLogFont(&lg);
		m_FontText.CreateFontIndirect(&lg);
	}
...
CString CTextProgressCtrl::GetPosText()
{
	CString StrRel;
	
	StrRel.Format(L"%d/%d", m_nPos, m_nMax);
	return StrRel;
}
...
void CTextProgressCtrl::GetRange32(int& nLower,int& nUpper)
{
	nLower = m_nMin;
	nUpper = m_nMax;
}
