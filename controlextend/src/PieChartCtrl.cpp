...
CPieChartCtrl::CPieChartCtrl()
{
	...
	m_nStartAngle = 0;
	m_colorLine = GetSysColor(COLOR_3DFACE);//RGB(0,0,0);
	...
	m_fontInfo.CreateFont(12, 0,0,0,FW_NORMAL, 0,0,0,
		DEFAULT_CHARSET, OUT_CHARACTER_PRECIS, CLIP_CHARACTER_PRECIS,
		DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, L"ËÎÌו");
	m_nTitleHeight = 0;
}
...
void CPieChartCtrl::OnPaint() 
{
	...
	for (int i=0; i<nCount; i++)
	{
		...
		int iItemAngle = (int)(pItem->m_fRate * 360.0f);
		nCurrectAngle += iItemAngle;
		if ( i == nCount - 1 )
		{
			nCurrectAngle = 360;
		}
		...
		if (iItemAngle>25)
		{
			int n = nCurrectAngle - (iItemAngle)/2;
			CPoint p;
			CountPoint(n, p, TRUE);
			CString str;
			str.Format(L"%.0f%%", 100 * pItem->m_fRate);
