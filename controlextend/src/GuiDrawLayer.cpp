...
DWORD GuiDrawLayer::m_SO= MAXUINT;
...
COLORREF GuiDrawLayer::GetRGBColorGrayText()
{
	...
	byGvalue =byGvalue>>1;
	byBvalue =byBvalue>>1;
	//lint --e{685} suppress "Relational operator '>' always evaluates to 'false'"
	int iMaximo=max(byRvalue,byGvalue);
	iMaximo=max(iMaximo,byBvalue);
	iMaximo= 255-iMaximo;
...
int GuiDrawLayer::GetTextWidth(CDC *pDC, const CString& strText, BOOL bHorz)
{
	if (NULL == pDC)
		return 0;
	...
	else
	{
		// 拿汉字做为宽度
		CSize size;
		size = pDC->GetTextExtent(_T("攀"));
		return size.cx;
	}
}

int GuiDrawLayer::GetTextHeight(CDC *pDC, const CString& strText, BOOL bHorz)
{
	if (NULL == pDC)
		return 0;
	...
	else
	{
		...
		TEXTMETRIC textMetric;
		pDC->GetTextMetrics(&textMetric);
		//int iFontHeight = textMetric.tmAscent;	// zhangbo # 使用tmAscent是因为实际显示的时候，总是太高， 取这个值小一点

#ifdef UNICODE	
		iHeight = textMetric.tmAscent * strText.GetLength();
#else // ANSI
		char acText[3] = {0};
...
void GuiDrawLayer::DrawText(CDC *pDC, const CString& strText, CRect rectText, UINT uiTextAlign, BOOL bHorz)
{
	if (NULL == pDC)	return;

	if (strText.GetLength() <= 0)	return;
	if (rectText.Height() <= 0 || rectText.Width() <= 0)	return;

	rectText.NormalizeRect();

	if (bHorz)
	{
		UINT uiAlign = DT_SINGLELINE;
		if (uiTextAlign & ETACenter)		uiAlign |= DT_CENTER;
		if (uiTextAlign & ETARight)			uiAlign |= DT_RIGHT;
		if (uiTextAlign & ETAVCenter)		uiAlign |= DT_VCENTER;
		if (uiTextAlign & ETABottom)		uiAlign |= DT_BOTTOM;

		pDC->DrawText(strText, &rectText, uiAlign);
	}
	else	// 竖着书写文字
	{
		int iTextWidth = GetTextWidth(pDC, strText, bHorz);
		int iTextHeight = GetTextHeight(pDC, strText, bHorz);
	
		int iX = rectText.left, iY = rectText.top;
		if (uiTextAlign & ETARight)
			iX = rectText.Width() - iTextWidth;
		else if (uiTextAlign & ETACenter)
			iX = rectText.left + (rectText.Width() - iTextWidth) / 2;

		if (uiTextAlign & ETABottom)
			iY = rectText.Height() - iTextHeight;
		else if (uiTextAlign & ETAVCenter)
			iY = rectText.top + (rectText.Height() - iTextHeight) / 2;

		TEXTMETRIC textMetric;
		pDC->GetTextMetrics(&textMetric);

		int iFontHeight = textMetric.tmAscent;

#ifdef UNICODE
		int iLen = strText.GetLength();
		for (int i = 0; i < iLen; i++)
		{
			CString strSingleChar(strText[i], 1);
			pDC->TextOut(iX, iY, strSingleChar);
		...
		for (int i = 0; i < iLen; i++)
		{
			...
			CString strSingleChar(acText, strlen(acText));
			pDC->TextOut(iX, iY, strSingleChar);
...
void CGradient::PrepareVertical(CDC *pDC,UINT RTop,UINT GTop,UINT BTop,UINT RBot,UINT GBot,UINT BBot)
{
	...
	double RectHeight=m_Size.cy/256.0f;
	...
	for(int ColorCount=0;ColorCount<NUM_COLORS;ColorCount++)
	{
		...
		CBrush br(RGB(rCount,gCount,bCount));
		m_dcMem->FillRect(CRect(0,ColorCount*(int)RectHeight,m_Size.cx,(ColorCount+1)*(int)RectHeight),&br);
		rCount+=rStep;
		gCount+=gStep;
		bCount+=bStep;
	}
...
void CGradient::PrepareHorizontal(CDC *pDC,UINT RLeft,UINT GLeft,UINT BLeft,UINT RRight,UINT GRight,UINT BRight)
{
	...
	{
		CBrush br(RGB(rCount,gCount,bCount));
		m_dcMem->FillRect(CRect(ColorCount*(int)RectWidth,0,(ColorCount+1)*(int)RectWidth,m_Size.cy),&br);
		rCount+=rStep;
		gCount+=gStep;
		bCount+=bStep;
	}
