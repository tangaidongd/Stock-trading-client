...
//static Win32Type g_Shell_Main=IsShellTypeMain();

void BCMenuData::SetAnsiString(LPCSTR szAnsiString)
{
	
	SetWideString(_MultiChar2Unicode(szAnsiString, CP_ACP).c_str());  //SK:  see MFC Tech Note 059
}
...
{
	return (g_Shell/*_Main*/>=Win95);
}

BOOL BCMenu::IsWinXPLuna()
{
	if(g_Shell/*_Main*/==WinXP){
...
void BCMenu::DrawItem_Win9xNT2000 (LPDRAWITEMSTRUCT lpDIS)
{
	...
	CDC* pDC = CDC::FromHandle(lpDIS->hDC);
	CRect rect;
	UINT state1 = (((BCMenuData*)(lpDIS->itemData))->nFlags);
...
BOOL BCMenu::AppendODMenuAMain(LPCSTR lpstrText,UINT nFlags,UINT nID,
                           int nIconNormal)
{	
	return AppendODMenuWMain((wchar_t*)_MultiChar2Unicode(lpstrText, CP_ACP).c_str(),nFlags,nID,nIconNormal);//SK: See MFC Tech Note 059
}


BOOL BCMenu::AppendODMenuWMain(wchar_t *lpstrText,UINT nFlags,UINT nID,
                           int nIconNormal)
{
...
{	
	return AppendODMenuWMain((wchar_t*)_MultiChar2Unicode(lpstrText, CP_ACP).c_str(),nFlags,nID,il,xoffset);
}
...
BOOL BCMenu::InsertODMenuAMain(UINT nPosition,LPCSTR lpstrText,UINT nFlags,UINT nID,
                           int nIconNormal)
{	
	return InsertODMenuWMain(nPosition,(wchar_t*)_MultiChar2Unicode(lpstrText, CP_ACP).c_str(),nFlags,nID,nIconNormal);
}


BOOL BCMenu::InsertODMenuWMain(UINT nPosition,wchar_t *lpstrText,UINT nFlags,UINT nID,
                           int nIconNormal)
{
	if(!(nFlags & MF_BYPOSITION)){
		...
		if(pMenu){
			return(pMenu->InsertODMenuWMain(iPosition,lpstrText,nFlags|MF_BYPOSITION,nID,nIconNormal));
		}
		else return(FALSE);
	}
	...
	//lint --e{429} suppress "Custodial pointer 'mdata' (line 1206) has not been freed or returned :m_MenuList已经托管仍然报出错误"
	mdata->SetWideString(lpstrText);    //SK: modified for dynamic allocation
...
BOOL BCMenu::InsertODMenuAMain(UINT nPosition,LPCSTR lpstrText,UINT nFlags,UINT nID,
                           CImageList *il,int xoffset)
{
	return InsertODMenuWMain(nPosition,(wchar_t*)_MultiChar2Unicode(lpstrText, CP_ACP).c_str(),nFlags,nID,il,xoffset);
}
                           ...
                           CImageList *il,int xoffset)
{
	if(!(nFlags & MF_BYPOSITION)){
		...
		BCMenu* pMenu = FindMenuOption(nPosition,iPosition);
		if(pMenu){
			return(pMenu->InsertODMenuWMain(iPosition,lpstrText,nFlags|MF_BYPOSITION,nID,il,xoffset));
		}
		else return(FALSE);
	}
...
BOOL BCMenu::ModifyODMenuAMain(const char * lpstrText,UINT nID,int nIconNormal)
{
	return ModifyODMenuWMain((wchar_t*)_MultiChar2Unicode(lpstrText, CP_ACP).c_str(),nID,nIconNormal);//SK: see MFC Tech Note 059
}

BOOL BCMenu::ModifyODMenuWMain(wchar_t *lpstrText,UINT nID,int nIconNormal)
{
	int nLoc;
...
BOOL BCMenu::ModifyODMenuAMain(const char * lpstrText,UINT nID,CImageList *il,int xoffset)
{	
	return ModifyODMenuWMain((wchar_t*)_MultiChar2Unicode(lpstrText, CP_ACP).c_str(),nID,il,xoffset);
}

BOOL BCMenu::ModifyODMenuWMain(wchar_t *lpstrText,UINT nID,CImageList *il,int xoffset)
{
	int nLoc;
	BCMenuData *mdata = NULL;
	CArray<BCMenu*,BCMenu*>bcsubs;
...
BOOL BCMenu::ModifyODMenuAMain(const char * lpstrText,UINT nID,CBitmap *bmp)
{	
	return ModifyODMenuWMain((wchar_t*)_MultiChar2Unicode(lpstrText, CP_ACP).c_str(),nID,bmp);
}

BOOL BCMenu::ModifyODMenuWMain(wchar_t *lpstrText,UINT nID,CBitmap *bmp)
{
	if(bmp){
		CImageList temp;
		...
		return ModifyODMenuWMain(lpstrText,nID,&temp,0);
	}
	return ModifyODMenuWMain(lpstrText,nID,NULL,0);
}

// courtesy of Warren Stevens
BOOL BCMenu::ModifyODMenuAMain(const char * lpstrText,UINT nID,COLORREF fill,COLORREF border,int hatchstyle,CSize *pSize)
{	
	return ModifyODMenuWMain((wchar_t*)_MultiChar2Unicode(lpstrText, CP_ACP).c_str(),nID,fill,border,hatchstyle,pSize);
}

BOOL BCMenu::ModifyODMenuWMain(wchar_t *lpstrText,UINT nID,COLORREF fill,COLORREF border,int hatchstyle,CSize *pSize)
{
	CWnd *pWnd = AfxGetMainWnd();            // Get main window
	CDC *pDC = pWnd->GetDC();              // Get device context
	SIZE sz;
	...
	pWnd->ReleaseDC(pDC);
	return ModifyODMenuWMain(lpstrText,nID,&bmp);
}


BOOL BCMenu::ModifyODMenuAMain(const char *lpstrText,const char *OptionText,
                          int nIconNormal)
{	
	return ModifyODMenuWMain((wchar_t*)_MultiChar2Unicode(lpstrText, CP_ACP).c_str(),(wchar_t*)_MultiChar2Unicode(OptionText, CP_ACP).c_str(),nIconNormal);//SK: see MFC  Tech Note 059
}

BOOL BCMenu::ModifyODMenuWMain(wchar_t *lpstrText,wchar_t *OptionText,
                           int nIconNormal)
{
	BCMenuData *mdata;
...
BCMenuData *BCMenu::NewODMenu(UINT pos,UINT nFlags,UINT nID,CString text)
{
	BCMenuData *mdata;
	...
	mdata->xoffset=-1;
#ifdef UNICODE
	mdata->SetWideString((LPCTSTR)text);//SK: modified for dynamic allocation
#else
	mdata->SetAnsiString(text);
#endif
	mdata->nFlags = nFlags;
	mdata->nID = nID;
		...
		if(AddBitmapToImageList(&imglist,nToolBar)){
			...
			for(int i=0;i<bar.GetCount();++i){
				...
				if(nID && GetMenuState(nID, MF_BYCOMMAND)
					...
					ModifyODMenuAMain(NULL,nID,&imglist,xoffset);
				}
			}
		}
	}
...
BOOL BCMenu::LoadMenu(LPCTSTR lpszResourceName)
{
	...
	do{
		...
		if(dwFlags & MF_POPUP){
			...
			m_Stack[m_Stack.GetUpperBound()]->AppendODMenuWMain(szCaption,uFlags,
...
void BCMenu::InsertSpaces(void)
{
	...
	for(i=0;i<numitems;++i){
		text=m_MenuList[i]->GetString();
		j=text.Find((char)9);
		newstring.Empty();
		if(j!=-1)newstring=text.Left(j);
		else newstring=text;
	...
	for(i=0;i<numitems;++i){
		...
		if(j!=-1){
			newstring.Empty();
			newstring=text.Left(j);			
			t=pDC->GetTextExtent(newstring.GetBuffer(),newstring.GetLength());
			while(t.cx<maxlength){
				newstring+=_T(' ');//SK: modified for Unicode correctness
			
				t=pDC->GetTextExtent(newstring.GetBuffer(),newstring.GetLength());
			}
			newstring+=text.Mid(j);
#ifdef UNICODE      
...
void BCMenu::LoadCheckmarkBitmap(int unselect, int iSelect)
{
	if(unselect>0 && iSelect>0){
		m_selectcheck=iSelect;
		...
		BOOL flag2=AddBitmapToImageList(checkmaps,iSelect);
		if(!flag1||!flag2){
...
//--------------------------------------------------------------------------
//[18.06.99 rj]
BOOL BCMenu::GetMenuText(UINT id, CString& tString, UINT nFlags/*= MF_BYPOSITION*/)
{
	BOOL returnflag=FALSE;
	...
	if(MF_BYPOSITION&nFlags){
		...
		if(id<=numMenuItems){
			tString=m_MenuList[id]->GetString();
			returnflag=TRUE;
		}
	}
	else{
		...
		if(NULL!=pMenu) returnflag = pMenu->GetMenuText(uiLoc,tString);
	}
	return(returnflag);
}
...
void BCMenu::SynchronizeMenu(void)
{
	...
	for(j=0;j<GetMenuItemCount();++j){
		...
		if(state&MF_POPUP){
			...
			GetMenuString(j,tString,MF_BYPOSITION);
			if(!mdata)mdata=NewODMenu(j,
				(state&0xFF)|MF_BYPOSITION|MF_POPUP|MF_OWNERDRAW,submenu,tString);
			else if(tString.GetLength()>0)
#ifdef UNICODE
				mdata->SetWideString(tString);  //SK: modified for dynamic allocation
#else
			mdata->SetAnsiString(tString);
#endif
		}
...
BOOL BCMenu::DeleteMenu(UINT uiId,UINT nFlags)
{
	if(MF_BYPOSITION&nFlags){
		...
		if(uint&MF_SEPARATOR && !(uint&MF_POPUP)){
			...
			if(NULL==pSubMenu){
				UINT uiCommandId = GetMenuItemID(uiId);
				for(int32 i=0;i<m_MenuList.GetSize(); i++){
					if(m_MenuList[i]->nID==uiCommandId){
						delete m_MenuList.GetAt(i);
						m_MenuList.RemoveAt(i);
						break;
...
BOOL BCMenu::AppendMenuAMain(UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,int nIconNormal)
{	
	return AppendMenuWMain(nFlags,nIDNewItem,(wchar_t*)_MultiChar2Unicode(lpszNewItem, CP_ACP).c_str(),nIconNormal);
}

BOOL BCMenu::AppendMenuWMain(UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,int nIconNormal)
{
	return AppendODMenuWMain(lpszNewItem,nFlags,nIDNewItem,nIconNormal);
}

BOOL BCMenu::AppendMenuAMain(UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CImageList *il,int xoffset)
{	
	return AppendMenuWMain(nFlags,nIDNewItem,(wchar_t*)_MultiChar2Unicode(lpszNewItem, CP_ACP).c_str(),il,xoffset);
}

BOOL BCMenu::AppendMenuWMain(UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,CImageList *il,int xoffset)
{
	return AppendODMenuWMain(lpszNewItem,nFlags,nIDNewItem,il,xoffset);
}

BOOL BCMenu::AppendMenuAMain(UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CBitmap *bmp)
{	
	return AppendMenuWMain(nFlags,nIDNewItem,(wchar_t*)_MultiChar2Unicode(lpszNewItem, CP_ACP).c_str(),bmp);
}

BOOL BCMenu::AppendMenuWMain(UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,CBitmap *bmp)
{
	if(bmp){
		CImageList temp;
		temp.Create(m_iconX,m_iconY,ILC_COLORDDB|ILC_MASK,1,1);
		...
		return AppendODMenuWMain(lpszNewItem,nFlags,nIDNewItem,&temp,0);
	}
	return AppendODMenuWMain(lpszNewItem,nFlags,nIDNewItem,NULL,0);
}

BOOL BCMenu::AppendMenuAMain(UINT nPosition,UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,int nIconNormal)
{	
	return InsertMenuWMain(nPosition,nFlags,nIDNewItem,(wchar_t*)_MultiChar2Unicode(lpszNewItem, CP_ACP).c_str(),nIconNormal);
}

BOOL BCMenu::InsertMenuWMain(UINT nPosition,UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,int nIconNormal)
{
	return InsertODMenuWMain(nPosition,lpszNewItem,nFlags,nIDNewItem,nIconNormal);
}

BOOL BCMenu::AppendMenuAMain(UINT nPosition,UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CImageList *il,int xoffset)
{
	return InsertMenuWMain(nPosition,nFlags,nIDNewItem,(wchar_t*)_MultiChar2Unicode(lpszNewItem, CP_ACP).c_str(),il,xoffset);
}

BOOL BCMenu::InsertMenuWMain(UINT nPosition,UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,CImageList *il,int xoffset)
{
	return InsertODMenuWMain(nPosition,lpszNewItem,nFlags,nIDNewItem,il,xoffset);
}

BOOL BCMenu::AppendMenuAMain(UINT nPosition,UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CBitmap *bmp)
{	
	return InsertMenuWMain(nPosition,nFlags,nIDNewItem,(wchar_t*)_MultiChar2Unicode(lpszNewItem, CP_ACP).c_str(),bmp);
}

BOOL BCMenu::InsertMenuWMain(UINT nPosition,UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,CBitmap *bmp)
{
	if(bmp){
		CImageList temp;
		temp.Create(m_iconX,m_iconY,ILC_COLORDDB|ILC_MASK,1,1);
		...
		return InsertODMenuWMain(nPosition,lpszNewItem,nFlags,nIDNewItem,&temp,0);
	}
	return InsertODMenuWMain(nPosition,lpszNewItem,nFlags,nIDNewItem,NULL,0);
}

//--------------------------------------------------------------------------
//[21.06.99 rj]
BCMenu* BCMenu::AppendODPopupMenuWMain(wchar_t *lpstrText)
{
	BCMenu* pSubMenu = new BCMenu;
	pSubMenu->m_unselectcheck=m_unselectcheck;
	pSubMenu->m_selectcheck=m_selectcheck;
	...
	pSubMenu->checkmapsshare=TRUE;
	pSubMenu->CreatePopupMenu();
	AppendODMenuWMain(lpstrText,MF_POPUP,(UINT)pSubMenu->m_hMenu, -1);
	return pSubMenu;
}

//--------------------------------------------------------------------------
//[21.06.99 rj]
BCMenu* BCMenu::AppendODPopupMenuAMain(LPCSTR lpstrText)
{	
	return AppendODPopupMenuWMain((wchar_t*)_MultiChar2Unicode(lpstrText, CP_ACP).c_str());
}
...
{	
	return GetSubBCMenu((wchar_t*)_MultiChar2Unicode(pText, CP_ACP).c_str());
}
...
{	
	return GetMenuPosition((wchar_t*)_MultiChar2Unicode(pText, CP_ACP).c_str());
}
...
{	
	return RemoveMenu((wchar_t*)_MultiChar2Unicode(pText, CP_ACP).c_str(), sPos);
}
...
{	
	return DeleteMenu((wchar_t*)_MultiChar2Unicode(pText, CP_ACP).c_str(), sPos);
}
...
// Larry Antram
BOOL BCMenu::SetMenuText(UINT id, CString tString, UINT nFlags/*= MF_BYPOSITION*/ )
{
	BOOL returnflag=FALSE;
	...
	if(MF_BYPOSITION&nFlags)
	{
		...
		if(id<=numMenuItems){
#ifdef UNICODE
			m_MenuList[id]->SetWideString((LPCTSTR)tString);
#else
			m_MenuList[id]->SetAnsiString(tString);
#endif
			returnflag=TRUE;
		}
	}
		...
		int uiLoc;
		BCMenu* pMenu = FindMenuOption(id,uiLoc);
		if(NULL!=pMenu) returnflag = pMenu->SetMenuText(uiLoc,tString);
	}
	return(returnflag);
}
