...
static const int32	KiSensitiveSize					= 1;				// 敏感尺寸
static const double KdF9Radion						= 0.99999999;		// F7 的比例

// 与该类相关的xml配置中相关信息
static const char * KStrElementValue				= "DualSplitWnd";	// 先不改名字.与以前的兼容	
...
{	
	...
	// 重新计算比率 - F7时保存会出现ClientArea太大，导致比例算错
	RecalcRatio();
	double dRatio = 0.0;
...
void CBiSplitterWnd::SetRadio(double dradio, bool32 bF9 /*= false*/)
{
	...
	else
	{
		if ( 0.0 == m_dF9Ratio )
		{
			...
			SetF9LockSplit(true);	// F7放大后自动锁定窗口分割
		}
		else
		{
			if (m_bRightBtn)
...
void CBiSplitterWnd::AdjustLayoutByRadioXml()
{
	...
	m_dRatioXml = 0.0; // 此处注意是从recalclayout调用，后面的set会导致调用recalclayout，防止重复调用
...
{
	// F7下，这个就是整个Frame的大小了 - 导致m_dRadio计算错误
	CRect RectClient;
...
void CBiSplitterWnd::OnGetMinMaxInfo( MINMAXINFO* lpMMI )
{
	...
	if ( NULL != lpMMI )
	{
		lpMMI->ptMinTrackSize = CPoint(0, 0);	// 不需要最小值
...
{
	// 事实上，如果f7后还能切割，则最后的f9窗口并不一定是view，有可能是view的集合 pane

	if ( m_dF9Ratio != 0.0 ) // 直接设置的0.0尽管比较？
	{
