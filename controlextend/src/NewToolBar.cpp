      ...
      if(m_dwStyle & CBRS_ORIENT_HORZ)
      {
        ...
        else
        {
          ...
          for(int i=0;i<nCountBtn;i++)
          {
            ...
            {
              CRect rcToolBar1(rect.left,lastItemRect.bottom,rect.right,itemRect.bottom);
              DrawGradient(pDC,rcToolBar1,clrBottomColor,clrUpperColor,false);
            }
            else
            {
              CRect rcToolBar1(rect.left,itemRect.top,rect.right,(itemRect.bottom+itemRect.top)/2);
              DrawGradient(pDC,rcToolBar1,clrUpperColor,clrMediumColor,false);

              rcToolBar1.top = rcToolBar1.bottom;
              rcToolBar1.bottom = itemRect.bottom;
              DrawGradient(pDC,rcToolBar1,clrMediumColor,clrBottomColor,false);
            } 
...
BOOL CNewToolBar::LoadHiColor(LPCTSTR lpszResourceName,COLORREF transparentColor/*=CLR_DEFAULT*/)
{
  ...
  if(hBitmap!=NULL)
  {
    //if(nColors>16)
    {
      ...
      case CNewMenu::STYLE_XP_2003_NOBORDER:
        {
          ...
          makeTransparentColor = MakeGrayAlphablend(&bitmap,110, blendcolor); 
        }
        break;
      ...
      m_ImageListDisabled.Add(&bitmap,makeTransparentColor); 
