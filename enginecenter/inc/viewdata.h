...
/////////////////////////////////////////////////////////////
//									   	
class CViewData : public CXTimerAgent, public CAbsCNewsManagerNotify, public CDataManagerNotify, public iAuthNotify, public COptimizeServerNotify, public CAbsCenterManager
{
public:
...
public:
	CViewData();
	//CViewData(CProxyAuthClientHelper* pAutherManager, CNewsManager* pNewsManager, HWND OwnerWnd);
	~CViewData();

public: 
	bool32			Construct();

	bool32          CreateModuleObjects();
	
	virtual void	Release();

	void			ConnectQuoteServer(E_NetType eNetType, COptimizeServerNotify *pNotify);
...
public:
	...
	CAbsDataManager* GetDataManager() { return m_pAbsDataManager; }
	CProxyAuthClientHelper* GetProxyAuthManager() { return m_pAutherManager; }
	CAbsNewsManager* GetNewsManager() { return m_pNewsManager; }
	CMerchManager& GetMerchManager() { return m_MerchManager;}
public:
	// 业务视图队列相关
...
public:
	...
	virtual void	OnDataServiceConnectStatus(int32 iServiceId, E_Connect_Status eConnectStatus); 
	virtual void	OnDataRequestTimeOut(int32 iMmiReqId); 
	virtual	void	OnDataCommResponse();
	virtual void	OnDataServerConnectStatus(int32 iCommunicationId, E_Connect_Status eConnectStatus);
...
public:
	...
	// from COffLineData与离线下载对外扩展接口
public:
	void OnRealtimePriceByOffLineData(const CRealtimePrice &RealtimePrice, int32 iCommunicationId, int32 iReqId);
	void OnDataRespMerchKLineByOffLineData(int iMmiReqId, IN CMmiRespMerchKLine *pMmiRespMerchKLine);
	// 清空本地数据
	void		ResetDataByOffLineData();
	// 取各种类型的最后下载时间. 其实就是 000001 的时间.
	CGmtTime	GetLastTimeByOffLineData(E_KLineTypeBase eType);
	// 下载行情数据
	void		DownLoadRealTimePriceDataByOffLineData(const T_DownLoadOffLine& stDownLoadOffLine);
	// 下载 K 线数据
	void		DownLoadKLineDataByOffLineData(const T_DownLoadOffLine& stDownLoadOffLine);
	// 下载 F10 数据
	void		DownLoadF10DataByOffLineData(const T_DownLoadOffLine& stDownLoadOffLine);
	// 开始下载
	void		StartDownLoadByOffLineData(const T_DownLoadOffLine& stDownLoadOffLine);
	// 停止下载
	void		StopDownLoadByOffLineData();
	void		SetNotifyByOffLineData(COffLineDataNotify* pNotify);
	bool		GetDownLoadStatusByOffLineData();
	void		SetDownLoadStatusByOffLineData(bool bDownLoadStatus);
	bool		GetTodayFlag();
	void		SetTodayFlag(bool bFlag);
	void		GetQuoteAuthJson(CString& strUserInfo);
	void		RequestStatusBarViewData();
...
public:
	void OnMsgTypeResponse(E_MsgTypeResponse emMsgType, WPARAM wParam, LPARAM lParam);
...
public:
	...
	CIndexChsStkMideCore *GetIndexChsStkMideCore();
	...
	void			GetAttendMerch(SmartAttendMerchArray& attendMerch,E_AttendMerchType type );
...
public:
	...
	const arrClientTradeTimeInfo &GetTradeTimeInfoArray() { return CMerchClientTradeTimeInfo::m_aClientTradeTimeInfo; }
...
public:
	//	离线登录
	bool32			OfflineLogin(IN CString& StrUserName, IN const CString& StrServerIP , IN const CString& StrKey, IN const CString& StrOrgCode);
...
public:
	...
	void			InitialDataServers(CProxyInfo& proxyInfo);
	CString			GetUserName();
	CString			GetUserPwd();
	CString			GetProduct();
...
public:
	E_LoginState GetLoginState() { return m_eLoginState; }
	void SetLoginState(E_LoginState eLoginState) { m_eLoginState = eLoginState; }

	bool32 GetKickOutState() { return m_bKickOut; }
	void SetKickOutState(bool32 bKickOut) { m_bKickOut = bKickOut; }

	float GetClosePrice()  {return m_fPriceClose; }
...
public:
	...
	COffLineData		*m_pOffLineData;		// 离线数据下载
