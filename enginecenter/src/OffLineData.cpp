...
CGmtTime COffLineData::GetLastTime(E_KLineTypeBase eType)
{
	...
	// 找到 000001 的时间作为标准
	if ( m_pViewData->m_pAbsDataManager->GetOfflineKLinesSnapshot(0, L"000001", eType, EOKTOfflineData, TimeBegin, TimeEnd, iCount) )
	{
		return TimeEnd;
	}
	...
	{	
		...
		for ( uint32 i = 0; i < aMerchsNow.size(); i++ )
		{
			...
			if ( !bFuture )
			{
				...
				m_pViewData->m_pAbsDataManager->RequestData(&ReqFinance, aMmiReqFinanceNodes, EDSCommunication);			
			}			
		...
		int32 iReVal = m_pViewData->m_pAbsDataManager->RequestData(&Req, aMmiReqNodes, EDSCommunication);	
...
bool32 COffLineData::WriteKLineData()
{
	...
	for( int32 i = 0; i < m_aKLinesToWrite.GetSize(); i++ )
	{
		...
		// 写数据
		m_pViewData->m_pAbsDataManager->WriteOfflineKLines(pDataNow->m_iMarketId, pDataNow->m_StrMerchCode, pDataNow->m_eKLineTypeBase, EOKTOfflineData, pDataNow->m_aKLines);
