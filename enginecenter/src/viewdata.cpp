...
//	m_pTradeBid = NULL;
	...
	// 商品离线数据下载
	m_pOffLineData = new COffLineData(this);
	//m_pOffLineData->SetNotify(this);

}

CViewData::~CViewData()
{
	if (NULL != m_pAutherManager)
	{
		if (m_pAutherManager->IsValid())
		{
			m_pAutherManager->CloseInstance();
		}
		DEL(m_pAutherManager);
	}
	...
	// 向DataManager注销自己
	m_pAbsDataManager->RemoveDataManagerNotify(this);
...
//	DEL(m_pAlarmCenter);
	...
	m_pAbsDataManager->Release();
	m_pNewsManager->Release();
}
...
bool32 CViewData::CreateModuleObjects()
{
	// 认证模块	
	m_pAutherManager = new CProxyAuthClientHelper();	

	if (NULL == m_pAutherManager)
		return false;

	if ( !m_pAutherManager->CreateInstance())		
		return false;	

	//DataEngine 模块
	m_pAbsDataManager = GetDataEngineObj();

	// 资讯模块
	m_pNewsManager			= GetNewsEngineObj();

	m_pAutherManager->GetInterface()->SetNotifyObj(this);
	//SetServiceDispose(m_pAutherManager->GetInterface()->GetServiceDispose());

	m_pServiceDispose = m_pAutherManager->GetInterface()->GetServiceDispose();
	m_pAbsDataManager->AddDataManagerNotify(this);
	m_pNewsManager->SetNewsManagerNotifyObj(this);
...
void CViewData::Release()
{
	delete this;
}

void CViewData::ConnectQuoteServer(E_NetType eNetType, COptimizeServerNotify *pNotify)
{

	// 资讯服务器暂未处理, 直接重连 - 单独
	//pDoc->m_pNewsManager->StartAllNewsCommunication(true);

	// 去掉 COptimizeServer 中的所有信息, 完全新的一个流程
	//	COptimizeServer::DelInstance();
	...
	COptimizeServer::Instance()->AddNotify(pNotify);
	COptimizeServer::Instance()->SetNetType(eNetType);

	//
	SetManualReConnectFlag(true);

	if(NULL == m_pAbsDataManager)
	{
		return;
	}

	// 开动所有的行情服务器:
	m_pAbsDataManager->StartAllService();	

	// 连接消息中心服务器
	//pDoc->m_pHttpClient->ConnectServer("192.168.0.233", 9002);

	//
	CArray<CAbsCommunicationShow *, CAbsCommunicationShow *> aServers;
	m_pAbsDataManager->GetCommnunicaionList(aServers);

	if ( aServers.GetSize() <= 0 )
	{
		m_pAbsDataManager->StopAllService();
		COptimizeServer::DelInstance();
		return;
	}

	//
	T_ServerInfo* pServerInfo = new T_ServerInfo[aServers.GetSize()];
	...
	for ( int32 i = 0; i < aServers.GetSize(); i++ )
	{
		...
		CAbsCommunicationShow * pServer = aServers.GetAt(i);
		...
		pServer->GetServerState(ServerState);

		//
		pServerInfo[iIndexParam].iID = ServerState.m_iServerID;
		pServerInfo[iIndexParam].enType = enSTQuote;
		lstrcpy(pServerInfo[iIndexParam].wszName, ServerState.m_StrServerName);	
		lstrcpy(pServerInfo[iIndexParam].wszAddr, ServerState.m_StrServerAddr);
		pServerInfo[iIndexParam].iPort = ServerState.m_uiServerPort;
		pServerInfo[iIndexParam].iHttpPort = ServerState.m_uiServerHttpPort;
		pServerInfo[iIndexParam].iOnLineCount = ServerState.m_iOnLineCount;
		pServerInfo[iIndexParam].iMaxCount = ServerState.m_iMaxCount;
		pServerInfo[iIndexParam].iGroup	= ServerState.m_iGroup;
		pServerInfo[iIndexParam].iSerNetID = ServerState.m_iNetType;
		//		lstrcpy(pServerInfo[iIndexParam].wszServerSummary, ServerState.m_StrServerSummary);
	...
	//
	COptimizeServer::Instance()->SetSortMode(false);
	COptimizeServer::Instance()->SetSortParams(pServerInfo, iIndexParam);	
	COptimizeServer::Instance()->SortQuoteServers();

	// 释放资源
	DEL_ARRAY(pServerInfo);
}
...
void CViewData::RequestViewData(IN CMmiCommBase *pMmiCommBase, IN int32 iCommunicationID /*= -1*/,IN int eDataSource  /* = EDSAuto*/)
{
	...
	case ECTReqAuth:  // 请求认证
		{
			CString &StrReqJson = ((CMmiReqAuth *)pMmiCommBase)->m_StrReqJson;
			PackQuoteAuthJson(StrReqJson);
...
void CViewData::OnViewReqMerchIndex(IN const CMmiReqMerchIndex* pMimiComBasePlugIn)
{
	...
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMimiComBasePlugIn, aMmiReqNodes);
}
...
void CViewData::OnAuthSuccessMain()
{
	if (NULL == m_LoginWnd)
	{		
		return;
...
void CViewData::InitialDataServers(CProxyInfo& proxyInfo)
{
	// 优选主站
	if ( NULL == m_pServiceDispose )
	{
	...
	CArray<T_ServerInfo, T_ServerInfo&> arrServerInfo; 
    arrServerInfo.RemoveAll();

	CArray<T_ServerInfo, T_ServerInfo&> arrNewsServerInfo; 
	arrNewsServerInfo.RemoveAll();
	...
	for ( i = 0; i < iCountServers; i++ )
	{
		...
		{
			arrNewsServerInfo.Add(pServerInfo[i]);
		}
		arrServerInfo.Add(pServerInfo[i]);

		/*T_ServerInfoByEngine tsIBEngine;
		tsIBEngine.iID			= pServerInfo[i].iID;
		tsIBEngine.enType		= (E_ServerTypeByEngine)(pServerInfo[i].enType);
		memcpy(tsIBEngine.wszName, pServerInfo[i].wszName, sizeof(wchar_t)*MAX_NAME_LEN);
		memcpy(tsIBEngine.wszAddr, pServerInfo[i].wszAddr, sizeof(wchar_t)*32);
		memcpy(tsIBEngine.wszSerKey, pServerInfo[i].wszSerKey, sizeof(wchar_t)*MAX_NAME_LEN);
		tsIBEngine.iPort		= pServerInfo[i].iPort;
		tsIBEngine.iHttpPort	= pServerInfo[i].iHttpPort;
		tsIBEngine.iOnLineCount = pServerInfo[i].iOnLineCount;
		tsIBEngine.iMaxCount	= pServerInfo[i].iMaxCount;
		tsIBEngine.iSerNetID	= pServerInfo[i].iSerNetID;
		tsIBEngine.iGroup		= pServerInfo[i].iGroup;

		arrServerInfo.Add(tsIBEngine);*/
	}
	...
	// 建立各个服务器的连接
	m_pAbsDataManager->SetStage(ECSTAGE_LoginInit);
	m_pAbsDataManager->InitAllService(arrServerInfo, proxyInfo, m_strUserName, m_strPassword);
	m_pAbsDataManager->StartAllService();

	CArray<CAbsCommunicationShow*, CAbsCommunicationShow*> aActiveServer;
	m_pAbsDataManager->GetCommnunicaionList(aActiveServer);
	for ( i = 0; i < aActiveServer.GetSize(); i++ )
	{
		...
		_MYTRACE(L"%d %s %s:%d",
			m_pAbsDataManager->GetCommunicationID(st.m_StrServerAddr, st.m_uiServerPort),
	...
	m_pNewsManager->InitAllNewsCommunication(proxyInfo, arrNewsServerInfo, m_strUserName, m_strPassword);
...
void CViewData::OnFinishSortServer()
{
	...
	for ( int32 ii = 0 ; ii < aServerSorted.GetSize() ; ii++ )
	{
		...
		_MYTRACE(L" => [%d] ID: %d 地址: %s 端口: %d", ss.m_iSortIndex, m_pAbsDataManager->GetCommunicationID(ss.m_StrServerAddr, ss.m_uiServerPort), ss.m_StrServerAddr.GetBuffer(), ss.m_uiServerPort);			
		ss.m_StrServerAddr.ReleaseBuffer();
	}

	m_pAbsDataManager->ReSetCommunicationIndex(aServerSorted);
	...
	for ( int32 i = 0; i < aServerSorted.GetSize(); i++ )
	{
		...
		bool32 bFirst = m_pAbsDataManager->BeFirstCommunication(stServerState.m_StrServerAddr, stServerState.m_uiServerPort);
		
		if ( bFirst )
		{
			int32 iCommunicationID = m_pAbsDataManager->GetCommunicationID(stServerState.m_StrServerAddr, stServerState.m_uiServerPort);
			bool32 bSevrConnected  = m_pAbsDataManager->IsServerConneted(stServerState.m_StrServerAddr, stServerState.m_uiServerPort);
			...
			m_pAbsDataManager->GetServerServiceList(stServerState.m_StrServerAddr, stServerState.m_uiServerPort, aServiceIDs);
...
void CViewData::OnFinishAllPing()
{
	...
	m_pAbsDataManager->StopServiceAfterSortServer();	
}
				...
				{					
					if ( bForceFree || !IsSomeIoViewAttendTheMerch(pMerch, EDSTKLine | EDSTTimeSale))
					{
						...
						*/		
						...
						m_pAbsDataManager->RemoveOfflineKLines(pMerch->m_MerchInfo.m_iMarketId, pMerch->m_MerchInfo.m_StrMerchCode, EKTBDay, EOKTVipData);
					}
				}
			}
		}
...
void CViewData::ReLogInOnServerDisconnected(int32 iCommunicationId)
{
	...
	{		
		...
		int iID = m_pAbsDataManager->GetCommunicationID(sState.m_StrServerAddr, sState.m_uiServerPort);
		if ( iID == iCommunicationId )
		{
			if ( m_pAbsDataManager->BeFirstCommunication(sState.m_StrServerAddr, sState.m_uiServerPort) )
			{
				bFirstServer = true;
				break;
			}
...
void CViewData::OnMsgServerLongTimeNoRecvData(int32 iCommunicationId, E_ReConnectType eType)
{
	...
	if ( NULL != m_LoginWnd )
	{
		if ( eType != ERCTPackTimeOut )
		{
			...
			// 清除队列
			m_pAbsDataManager->RemoveHistoryReqOfSpecifyServer(iCommunicationId);
			
			// 重连一下
			m_pAbsDataManager->StartService(iCommunicationId);
...
void CViewData::OnDataServiceConnectStatus( int32 iServiceId, E_Connect_Status eConnectStatus )
{
	// 多线程变单线程处理
	if(EG_CNT_STATUS_DIS_CONNECT == eConnectStatus)
	{
		PostMessage(m_OwnerWnd, UM_ViewData_OnDataServiceDisconnected, (WPARAM)iServiceId, 0);
	}
	else if(EG_CNT_STATUS_CONNECT == eConnectStatus)
	{
		PostMessage(m_OwnerWnd, UM_ViewData_OnDataServiceConnected, (WPARAM)iServiceId, 0);
	}
}

void CViewData::OnDataServerConnectStatus( int32 iCommunicationId, E_Connect_Status eConnectStatus )
{
	// 多线程变单线程处理
	if(EG_CNT_STATUS_CONNECT == eConnectStatus)
	{
		PostMessage(m_OwnerWnd, UM_ViewData_OnDataServerConnected, (WPARAM)iCommunicationId, 0);
	}
	else if(EG_CNT_STATUS_DIS_CONNECT == eConnectStatus)
	{	
		PostMessage(m_OwnerWnd, UM_ViewData_OnDataServerDisconnected, (WPARAM)iCommunicationId, 0);
	}
}
...
void CViewData::OnMsgServerConnected(int32 iCommunicationId)
{
	...
	if (m_bDirectToQuote)
	{
		...
		int32 iCommunicationID = m_pAbsDataManager->GetCommunicationID(stLoginBk.m_StrQuoterSvrAdd, stLoginBk.m_iQuoterSvrPort);
		bool32 bSevrConnected  = m_pAbsDataManager->IsServerConneted(stLoginBk.m_StrQuoterSvrAdd, stLoginBk.m_iQuoterSvrPort);
...
{	
	...
	if (ELSLoginSuccess == m_eLoginState)
		iProcessPackageCount = KProcessPackageCountPerMsg;

	while (iProcessPackageCount--)
	{
		CMmiRespNode *pMmiRespNode = m_pAbsDataManager->PeekAndRemoveFirstResp();
			...
			else
			{
				if (EPFTWeight == pMmiRespPublicFile->m_ePublicFileType)	// 如果是除权数据， 保存该数据
				{
					if (CMerch::ReadWeightDatasFromBuffer(pMmiRespPublicFile->m_pcBuffer, pMmiRespPublicFile->m_iValidDataLen, pMerch->m_aWeightDatas))
					{
						...
						// 尝试保存到文件中
						CString StrFilePath;
						if (m_pAbsDataManager->GetPublishFilePath(EPFTWeight, pMmiRespPublicFile->m_iMarketId, pMmiRespPublicFile->m_StrMerchCode, StrFilePath))
						{
				...
				else if ( EPFTF10 == pMmiRespPublicFile->m_ePublicFileType )	
				{
					...
					if ( CMerch::ReadFinanceDataFromBuffer(pMmiRespPublicFile->m_pcBuffer, pMmiRespPublicFile->m_iValidDataLen, *pMerch->m_pFinanceData) )
					{
						...
						CString StrFilePath;
						if ( m_pAbsDataManager->GetPublishFilePath(EPFTF10, pMmiRespPublicFile->m_iMarketId, pMmiRespPublicFile->m_StrMerchCode, StrFilePath))
						{
							_tcheck_if_mkdir(StrFilePath.LockBuffer());
							StrFilePath.UnlockBuffer();
...
void CViewData::OnDataRespClientTradeTime(int iMmiReqID, IN const CMmiRespTradeTime* pMmiRespTradeTime)
{
	...
	CMerchClientTradeTimeInfo::m_aClientTradeTimeInfo.Copy(pMmiRespTradeTime->m_aClientTradeTimeInfo);
...
void CViewData::OnAllMarketInitializeSuccess()
{
	...
	// 初始化完毕，场景改变
	m_pAbsDataManager->SetStage(ECSTAGE_QuoteSupplier);
...
void CViewData::OnViewReqAuth(IN const CMmiReqAuth *pMmiReqAuth, IN int32 iCommunicationID /*= -1*/)
{
	...
	// 向服务器发认证请求
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqAuth, aMmiReqNodes, EDSCommunication, iCommunicationID);
}
...
void CViewData::OnViewReqBase(IN CMmiCommBase *pMmiCommBase, IN int32 iCommunicationID /* = -1 */,IN int eDataSource /* = EDSAuto */)
{
	...
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData(pMmiCommBase, aMmiReqNodes, (E_DataSource)eDataSource,iCommunicationID);
...
void CViewData::OnViewReqMarketInfo(IN const CMmiReqMarketInfo *pMmiReqMarketInfo)
{
	...
	// 向服务器发请求
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	int32 iRet = m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqMarketInfo, aMmiReqNodes);
...
void CViewData::OnViewReqMarketSnapshot(IN const CMmiReqMarketSnapshot *pMmiReqMarketSnapshot)
{
	...
	// 向服务器发请求
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqMarketSnapshot, aMmiReqNodes);
}
...
void CViewData::OnViewReqBroker(IN const CMmiReqBroker *pMmiReqBroker)
{
	...
	// 向服务器发请求
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqBroker, aMmiReqNodes);
}
...
//	RGUARD(LockSingle, pMarket->m_LockMerchList, LockMerchList);
	...
	// 向服务器发请求
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	int32 iRet = m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqMerchInfo, aMmiReqNodes);
...
{ 
	...
	else if ( EPFTF10 == pMmiReqPublicFile->m_ePublicFileType )
	{
		...
		if ( m_pAbsDataManager->GetPublishFilePath(EPFTF10, pMerch->m_MerchInfo.m_iMarketId , pMerch->m_MerchInfo.m_StrMerchCode, StrFilePath) )
		{
			// 读数据:
			CFile file;
	...
	// 向服务器发请求
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqPublicFile, aMmiReqNodes);
}
...
void CViewData::OnViewReqMerchExtendData(IN const CMmiReqMerchExtendData *pMmiReqMerchExtendData)
{
	...
	// 向服务器发请求
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqMerchExtendData, aMmiReqNodes);
}
...
void CViewData::OnViewReqMerchF10(IN const CMmiReqMerchF10 *pMmiReqMerchF10)
{
	...
	// 向服务器发请求
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqMerchF10, aMmiReqNodes);
}
...
{	
	...
	if ( m_bOffLine )
	{
		if ( aMerchs.GetSize() > 0 )
		{
			...
			// 向服务器发请求
			CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
			int32 iRet = m_pAbsDataManager->RequestData((CMmiCommBase *)&Req, aMmiReqNodes, EDSOfflineData);
			if (iRet)
			{
				// ...
			}
	...
	else
	{
		...
		if (aMerchs.GetSize() > 0)
		{
			while (aMerchs.GetSize() > 0)
			{
				...
				// 向服务器发请求
				CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
				int32 iRet = m_pAbsDataManager->RequestData((CMmiCommBase *)&Req, aMmiReqNodes);
...
void CViewData::OnViewRegisterPushPrice(IN const CMmiRegisterPushPrice *pMmiRegisterPushPrice)
{
	...
	if (m_bDirectToQuote)
	{
		...
		CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
		int32 iRet = m_pAbsDataManager->RequestData((CMmiCommBase *)&Req, aMmiReqNodes, EDSCommunication);
		if (iRet)
		{
			// ....
		}
	...
	for (int32 i = 0; i < pMmiRegisterPushPrice->m_aMerchMore.GetSize() + 1; i++)
	{
		...
		else
		{
			...
			if (itFind == aPushingRealtimePrices.end())	// 没有找到该推送， 则重新查找一个能提供该商品实时行情推送的服务器
			{
				...
				{
					CArray<int32, int32> aCommunicationIds;
					if (!bHasInitServiceList)
					{
						m_pAbsDataManager->GetActiveCommunicationByService(iServiceId, aCommunicationIds);
						bHasInitServiceList = true;
					}

					if(aCommunicationIds.GetSize() > 0)
					{
					     MerchAndCommunication.m_iCommunicationId = aCommunicationIds[0];
					}
	...
	if (aMerchs.GetSize() > 0)
	{
		while (aMerchs.GetSize() > 0)
		{
			...
			CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
			int32 iRet = m_pAbsDataManager->RequestData((CMmiCommBase *)&Req, aMmiReqNodes, EDSAuto, iCommunicationId);
...
void CViewData::OnViewRegisterPushTick(IN const CMmiRegisterPushTick *pMmiRegisterPushTick)
{
	...
	if (m_bDirectToQuote)
	{
		...
		CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
		m_pAbsDataManager->RequestData((CMmiCommBase *)&Req, aMmiReqNodes, EDSCommunication);
...
void CViewData::OnViewReqNetTest(IN const CMmiReqNetTest *pMmiReqNetTest, IN int32 iCommunicationID)
{
	...
	// 向服务器发测速请求
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqNetTest, aMmiReqNodes, EDSCommunication, iCommunicationID);	
}
...
void CViewData::OnViewReqGeneralNormal(IN const CMmiReqGeneralNormal *pMmiReqGeneralNormal, IN int32 iCommunicationID)
{
	...
	// 向服务器发测速请求
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqGeneralNormal, aMmiReqNodes, EDSCommunication, iCommunicationID);
}
...
void CViewData::OnViewReqGeneralFiniance(IN const CMmiReqGeneralFinance *pMmiReqGeneralFinance, IN int32 iCommunicationID)
{
	...
	// 向服务器发测速请求
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqGeneralFinance, aMmiReqNodes, EDSCommunication, iCommunicationID);
}
...
void CViewData::OnViewReqMerchTrendIndex(IN const CMmiReqMerchTrendIndex *pMmiReqMerchTrendIndex, IN int32 iCommunicationID)
{
	...
	// 向服务器发测速请求
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqMerchTrendIndex, aMmiReqNodes, EDSCommunication, iCommunicationID);	
}
...
void CViewData::OnViewReqMerchAuction(IN const CMmiReqMerchAution *pMmiReqMerchAution, IN int32 iCommunicationID)
{
	...
	// 向服务器发测速请求
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqMerchAution, aMmiReqNodes, EDSCommunication, iCommunicationID);	
}
...
void CViewData::OnViewReqAddPushGeneralNormal(IN const CMmiReqAddPushGeneralNormal *pMmiReqAddPushGeneralNormal, IN int32 iCommunicationID)
{
	...
	// 向服务器发测速请求
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqAddPushGeneralNormal, aMmiReqNodes, EDSCommunication, iCommunicationID);
}
...
void CViewData::OnViewReqUpdatePushGeneralNormal(IN const CMmiReqUpdatePushGeneralNormal* pMmiReqUpdatePushGeneralNormal, IN int32 iCommunicationID)
{
	...
	// 向服务器发测速请求
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqUpdatePushGeneralNormal, aMmiReqNodes, EDSCommunication, iCommunicationID);
}
...
void CViewData::OnViewReqDelPushGeneralNormal(IN const CMmiReqDelPushGeneralNormal* pMmiReqDelPushGeneralNormal, IN int32 iCommunicationID)
{
	...
	// 向服务器发测速请求
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqDelPushGeneralNormal, aMmiReqNodes, EDSCommunication, iCommunicationID);
}
...
void CViewData::OnViewReqAddPushMerchTrendIndex(IN const CMmiReqAddPushMerchTrendIndex *pMmiReqAddPushMerchTrendIndex, IN int32 iCommunicationID)
{
	...
	// 向服务器发测速请求
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqAddPushMerchTrendIndex, aMmiReqNodes, EDSCommunication, iCommunicationID);
}
...
void CViewData::OnViewReqUpdatePushMerchTrendIndex(IN const CMmiReqUpdatePushMerchTrendIndex *pMmiReqUpdatePushMerchTrendIndex, IN int32 iCommunicationID)
{
	...
	// 向服务器发测速请求
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqUpdatePushMerchTrendIndex, aMmiReqNodes, EDSCommunication, iCommunicationID);
}
...
void CViewData::OnViewReqDelPushMerchTrendIndex(IN const CMmiReqDelPushMerchTrendIndex *pMmiReqDelPushMerchTrendIndex, IN int32 iCommunicationID)
{
	...
	// 向服务器发测速请求
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqDelPushMerchTrendIndex, aMmiReqNodes, EDSCommunication, iCommunicationID);
}
...
{	if ( NULL == pMmiReqFundHold )
	...
	//
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqFundHold, aMmiReqNodes, EDSCommunication, iCommunicationID);
}
...
void CViewData::OnViewReqMinuteBS(IN const CMmiReqMinuteBS *pMmiReqMinuteBS, IN int32 iCommunicationID)
{
	...
	//
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqMinuteBS, aMmiReqNodes, EDSAuto, iCommunicationID);
}
...
void CViewData::OnViewReqAddPushMinuteBS(IN const CMmiReqAddPushMinuteBS *pMmiReqAddPushMinuteBS, IN int32 iCommunicationID)
{
	...
	//
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqAddPushMinuteBS, aMmiReqNodes, EDSCommunication, iCommunicationID);
}
...
void CViewData::OnViewReqUpdatePushMinuteBS(IN const CMmiReqUpdatePushMinuteBS *pMmiReqUpdatePushMinuteBS, IN int32 iCommunicationID)
{
	...
	//
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqUpdatePushMinuteBS, aMmiReqNodes, EDSCommunication, iCommunicationID);
}
...
void CViewData::OnViewReqDelPushMinuteBS(IN const CMmiReqDelPushMinuteBS *pMmiReqDelPushMinuteBS, IN int32 iCommunicationID)
{
	...
	//
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqDelPushMinuteBS, aMmiReqNodes, EDSCommunication, iCommunicationID);
}
...
void CViewData::OnReqClientTradeTime(IN const CMmiReqTradeTime* pMmiReqTradeTime, IN int32 iCommunicationID)
{
	...
	//
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqTradeTime, aMmiReqNodes, EDSCommunication, iCommunicationID);
}
...
void CViewData::OnReqAuthPlugInHeart(IN const CMmiReqAuthPlugInHeart* pMimiComBase, IN int32 iCommunicationID)
{
	...
	//
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMimiComBase, aMmiReqNodes, EDSCommunication, iCommunicationID);
}
...
void CViewData::OnViewReqMerchKLine(IN const CMmiReqMerchKLine *pMmiReqMerchKLine)
{
	...
	if (!pMerch->FindMerchKLineNode(pMmiReqMerchKLine->m_eKLineTypeBase, iPosKLineFound, pMerchKLineNodeFound) || pMerchKLineNodeFound->m_KLines.GetSize() <= 0)
	{
		// 向数据中心请求数据
		CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
		m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqMerchKLine, aMmiReqNodes);
	}
	else
	{
		// zhangbo 0625 #待优化
...
void CViewData::OnViewReqMerchTimeSales(IN const CMmiReqMerchTimeSales *pMmiReqMerchTimeSales)
{
	...
	// 
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqMerchTimeSales, aMmiReqNodes);
}
...
void CViewData::OnViewReqReport(IN const CMmiReqReport *pMmiReqReport)
{
	...
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqReport, aMmiReqNodes);
}
...
void CViewData::OnViewReqBlockReport(IN const CMmiReqBlockReport *pMmiReqBlockReport)
{
	...
	CArray<CMmiReqNode*, CMmiReqNode*> aMmiReqNodes;
	m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqBlockReport, aMmiReqNodes);
}
...
CIndexChsStkMideCore *CViewData::GetIndexChsStkMideCore()
{
	return m_pIndexChsStkMideCore;
}
...
bool CViewData::IsRevFinishByIndexChsStk()
{
	if (-1 == m_iNumsOfIndexChsStkReq)
	{
		return false;
	}

	if (m_iNumsOfIndexChsStkHaveRecved == m_iNumsOfIndexChsStkReq)
	{
		return true;
	}
...
//	CGGTongDoc* pDoc = (CGGTongDoc*)AfxGetDocument();
	...
	else
	{
		...
		for ( int32 i = m_aQuoteServerAll.GetSize() -1; i >= 0; i-- )
		{
			...
			// 看这个
			int32 iID = m_pAbsDataManager->GetCommunicationID(stServerInfo.wszAddr, stServerInfo.iPort);
			if ( iID == iCommunicationId )
			{
	...
	else
	{
		...
		if (m_aQuoteServerAll.GetSize() > 0)
		{
			for ( int32 i = 0; i < m_aQuoteServerAll.GetSize(); i++ )
			{
				...
				int32 iCommID  = m_pAbsDataManager->GetCommunicationID(StrAdd, iPort);
				...
				// 停止这个行情, 防止过会儿连上了
				m_pAbsDataManager->StopService(iCommID);
			}
...
bool32 CViewData::BeLocalOfflineDataEnough(E_KLineTypeBase eType)
{
	...
	{
		// 找到 000001 的时间作为标准
		if ( m_pAbsDataManager->GetOfflineKLinesSnapshot(0, L"000001", eType, EOKTOfflineData, TimeBegin, TimeEnd, iCount) )
		{			
			//
			CGmtTime TimeNow = GetServerTime();
...
void CViewData::DisconnectQuoteServer()
{
	m_pAbsDataManager->StopAllService();
...
bool32 CViewData::DirectConnectQuote()
{
	...
	//// 建立各个服务器的连接
	//m_pAbsDataManager->SetStage(ECSTAGE_LoginInit);
	//m_pAbsDataManager->InitDirectService(m_proxyInfo, m_strUserName, m_strPassword, stLoginBk.m_StrQuoterSvrAdd, stLoginBk.m_iQuoterSvrPort, aServiceIDs);
	//m_pAbsDataManager->StartAllService();
...
void CViewData::GetAttendMerch(SmartAttendMerchArray& attendMerch,E_AttendMerchType type )
{
// 	if (type == EA_Alarm)
// 	{
// 		attendMerch = m_aAlarmAttendMerchs;
// 	}
// 	else if (type == EA_RecordData)
// 	{
// 		attendMerch = m_aRecordDataCenterAttendMerchs;
// 	}
// 	else if (type == EA_StatucBar)
// 	{
// 		attendMerch = m_StatucBarAttendMerchs;
// 	}
// 	else if (type == EA_Arbitrage)
// 	{
// 		attendMerch = m_aArbitrageAttendMerchs;
// 	}
// 	else if (type == EA_Choose)
// 	{
// 		attendMerch = m_aIndexChsDataAttendMerchs;
// 	}
// 	else if (type == EA_SimulateTrade)
// 	{
// 		attendMerch = m_aSimulateTradeAttendMerchs;
// 	}
// 
...
bool32 CViewData::OfflineLogin(IN CString& StrUserName, IN const CString& StrServerIP , IN const CString& StrKey, IN const CString& StrOrgCode)
{	
	// 定时器不要了
	InitPush(false);
	...
	// 设置请求类型
	m_pAbsDataManager->ForceRequestType(EDSCommunication);

	vector<unsigned int> vtPort;
	//	连接认证服务器，为请求网络市场数据做准备
	if(!m_pAutherManager->GetInterface()->ConnectServer(StrServerIP,vtPort))
	{
		ASSERT(0);
		return false;
	}
	...
	// 请求服务器市场相关信息
	m_pAutherManager->GetInterface()->ReqQueryServerInfo(StrKey, StrOrgCode, 48, true, 1);
 ...
 */
    ...
    InitPush(false);
     ...
     // 设置请求类型
     m_pAbsDataManager->ForceRequestType(EDSOfflineData);
...
// 清空本地数据
void CViewData::ResetDataByOffLineData()
{
	if (m_pOffLineData)
	{
		m_pOffLineData->ResetData();
	}
}

// 取各种类型的最后下载时间. 其实就是 000001 的时间.
CGmtTime CViewData::GetLastTimeByOffLineData(E_KLineTypeBase eType)
{
	if (m_pOffLineData)
	{
		return m_pOffLineData->GetLastTime(eType);
	}
	return NULL;
}

// 下载行情数据
void CViewData::DownLoadRealTimePriceDataByOffLineData(const T_DownLoadOffLine& stDownLoadOffLine)
{
	if (m_pOffLineData)
	{
		m_pOffLineData->DownLoadRealTimePriceData(stDownLoadOffLine);
	}
}

// 下载 K 线数据
void CViewData::DownLoadKLineDataByOffLineData(const T_DownLoadOffLine& stDownLoadOffLine)
{
	if (m_pOffLineData)
	{
		m_pOffLineData->DownLoadKLineData(stDownLoadOffLine);
	}
}

// 下载 F10 数据
void CViewData::DownLoadF10DataByOffLineData(const T_DownLoadOffLine& stDownLoadOffLine)
{
	if (m_pOffLineData)
	{
		m_pOffLineData->DownLoadF10Data(stDownLoadOffLine);
	}
}

// 开始下载
void CViewData::StartDownLoadByOffLineData(const T_DownLoadOffLine& stDownLoadOffLine)
{
	if (m_pOffLineData)
	{
		m_pOffLineData->StartDownLoad(stDownLoadOffLine);
	}
}

// 停止下载
void CViewData::StopDownLoadByOffLineData()
{
	if (m_pOffLineData)
	{
		m_pOffLineData->StopDownLoad();
...
void CViewData::SetNotifyByOffLineData(COffLineDataNotify* pNotify)
{
	if (m_pOffLineData)
	{
		m_pOffLineData->SetNotify(pNotify);
	}
}

bool CViewData::GetDownLoadStatusByOffLineData()
{
	return m_bDownLoading;
}
void CViewData::SetDownLoadStatusByOffLineData(bool bDownLoadStatus)
{
	m_bDownLoading = bDownLoadStatus;
}
...
void CViewData::GetQuoteAuthJson(CString& strUserInfo)
{
	PackQuoteAuthJson(strUserInfo);
}
...
void CViewData::OnMsgTypeResponse(E_MsgTypeResponse emMsgType, WPARAM wParam, LPARAM lParam)
{
	switch(emMsgType)
	{
	case EMSG_OnMsgRealtimeTickUpdate:
		OnMsgRealtimeTickUpdate((CMerch*)wParam);
		break;
	case EMSG_OnMsgRealtimePriceUpdate:
		OnMsgRealtimePriceUpdate((CMerch*)wParam);
		break;
	case EMSG_OnMsgRealtimeLevel2Update:
		OnMsgRealtimeLevel2Update((CMerch*)wParam);
		break;
	case EMSG_OnMsgPublicFileUpdate:
		OnMsgPublicFileUpdate((CMerch*)wParam, (E_PublicFileType)lParam);
		break;
	case EMSG_OnMsgOnMerchTrendIndexUpdate:
		OnMsgOnMerchTrendIndexUpdate((CMerch*)wParam);
		break;
	case EMSG_OnMsgOnMerchMinuteBSUpdate:
		OnMsgOnMerchMinuteBSUpdate((CMerch* )wParam);
		break;
	case EMSG_OnMsgOnMerchAuctionUpdate:
		OnMsgOnMerchAuctionUpdate((CMerch* )wParam);
		break;
	case EMSG_OnMsgOnGeneralNormalUpdate:
		OnMsgOnGeneralNormalUpdate((CMerch* )wParam);
		break;
	case EMSG_OnMsgOnGeneralFinanaceUpdate:
		OnMsgOnGeneralFinanaceUpdate((CMerch* )wParam);
		break;
	case EMSG_OnMsgNewsServerDisconnected:
		OnMsgNewsServerDisconnected((int32)wParam);
		break;
	case EMSG_OnMsgNewsServerConnected:
		OnMsgNewsServerConnected((int32)wParam);
		break;
	case EMSG_OnMsgNewsResponse:
		OnMsgNewsResponse();
		break;
	case EMSG_OnMsgNewsListUpdate:
		OnMsgNewsListUpdate();
		break;
	case EMSG_OnMsgLandMineUpdate:
		OnMsgLandMineUpdate((CMerch* )wParam);
		break;
	case EMSG_OnMsgKLineNodeUpdate:
		OnMsgKLineNodeUpdate((CMerch* )wParam);
		break;
	case EMSG_OnMsgF10Update:
		OnMsgF10Update((CMerch* )wParam);
		break;
	case EMSG_OnMsgClientTradeTimeUpdate:
		OnMsgClientTradeTimeUpdate();
		break;
	case EMSG_OnMsgAuthPlugInHeartResp:
		OnMsgAuthPlugInHeartResp((int32)wParam);
		break;
	case EMSG_OnMsgTimesaleUpdate:
		OnMsgTimesaleUpdate((CMerch* )wParam);
		break;
	case EMSG_EMSGOnMsgViewDataOnTimerFreeMemory:
		OnTimerFreeMemory();
		break;
	case EMSG_OnMsgViewDataOnTimerInitializeMarket:
		OnTimerInitializeMarket();
		break;
	case EMSG_OnMsgViewDataOnTimerSyncPushing:
		OnTimerSyncPushing();
		break;	
	}
}
...
// 
ENGINECENTER_DLL_EXPORT CAbsCenterManager* GetCenterManagerObj(int iType)
{
	// 不兼容此前任何版本，不能提供大于此版本的服务
	if (g_iCenterManagerVer < iType)
	{
		char szErr[200];
		sprintf_s(szErr, "GetProxyAuthClientObj: request version not match: %d!=%d\r\n", iType, g_iCenterManagerVer);
		OutputDebugStringA(szErr);
		return NULL;
	}

	CViewData *pCenterManager = new CViewData();
	if (!pCenterManager->Construct())
	{
		DEL(pCenterManager);
		return NULL;
	}

	return pCenterManager;
}
