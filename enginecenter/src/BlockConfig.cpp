		...
		if ( pNewCollection != NULL )
		{
			...
			while ( NULL != pTiEleBlock )
			{
				...
				if ( NULL != pAttriValue )
				{
					...
					{
						if ( m_pViewData->GetMerchManager().FindMerch(StrMerchCode, iMarketId, pMerch) )	// 没有的商品不加入
						{
...
void CBlockConfig::FillMerchByMaskString( const CString &StrMask, int32 iMarketId, OUT MerchArray &aMerchsFind )
{
	...
	if ( !m_pViewData->GetMerchManager().FindMarket(iMarketId, pMarket) || NULL == pMarket )
	{
		return;
	}
...
bool32 CBlockDataPseudo::GetColValue( E_Header eHeader, OUT void *pValue ) const
{
	// 数组比map要好 - 先将就着
	if ( NULL == pValue )
	{
