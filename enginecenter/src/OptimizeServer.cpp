...
bool32 COptimizeServer::RequestNetTest(IN const CMmiReqNetTest *pMmiReqNetTest, IN int32 iCommunicationID, OUT int32& iReqID)
{
	...
	int32 iReVal = m_pViewData->m_pAbsDataManager->RequestData((CMmiCommBase *)pMmiReqNetTest, aMmiReqNodes, EDSCommunication, iCommunicationID);	
...
void COptimizeServer::PingAllServers()
{
	...
	for ( int32 i = 0; i < m_aServersFromAuth.GetSize(); i++ )
	{
		...
		// 得到指定的行情服务器:		
		bool32 bSevrConnected  = m_pViewData->m_pAbsDataManager->IsServerConneted(stServerState.m_StrServerAddr, stServerState.m_uiServerPort);
		int32 iCommunicationID = m_pViewData->m_pAbsDataManager->GetCommunicationID(stServerState.m_StrServerAddr, stServerState.m_uiServerPort);
...
void COptimizeServer::OnDataServerDisconnected( int32 iCommunicationId )
{
	...
	for ( int32 i = m_aServerPingInfo.GetSize()-1; i >= 0; --i )
	{
		...
		//
		int32 iComm = m_pViewData->m_pAbsDataManager->GetCommunicationID(stServerSortPing.m_stServer.m_StrServerAddr, stServerSortPing.m_stServer.m_uiServerPort);
