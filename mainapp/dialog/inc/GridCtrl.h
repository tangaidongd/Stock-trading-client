...
#define GVSDS_DEFAULT			(0)		// 选择单元格默认绘制方式
#define GVSDS_BOTTOMLINE		(1)		// 选择单元格底部高亮线绘制方式
...
class CGridCtrl : public CWnd
{
	...
	friend class CIoViewReportSelect;		// 先临时这么做吧
...
//	BOOL	 BeShowGridLine()					  { return m_bShowGridLine; }
	...
	void	 SetFixColBkColor(COLORREF clr)	 { m_cellFixedColDef.SetBackClr(clr); }	// lcq add 20131225 单独设置行首字段背景
...
//	void SetBaseLineColor ( COLORREF color );
	...
	void SetRemoveSelectClickBlank(BOOL bRemove=FALSE) { m_bRemoveSelectClickBlank = bRemove;}	//lcq add 20130117点击空白区域，取消上次选中项
	void SetListHeaderCanClick(BOOL bCan=TRUE) {m_bListHeaderCanNotClick = bCan;}			//lcq add 20130117表头可以被选中,选择行时表头会凹进去
///////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////
	...
	void    SetNeedShowFixSpliter(bool bNeed) { m_bNeedShowFixRowSpliter = bNeed;}    //是否显示固定行的分隔条
...
public:
	...
	// 一种特殊的自适应列宽的方法,因为自带的有许多问题不符合此处要求,又不能随便改动,以免影引其它地方出现错误
...
protected:
	...
	void UpdateSelectedRows() const;	// 更新选择的行的数据
public:
...
protected:
	...
	// fangz 1023# 自选股排序,最后一行不参加.增加一个设定排序的Row 范围的函数
...
public:
	...
	void EnableOriginalScroll(BOOL bEnable = FALSE) { m_bOriginalScroll = TRUE; }; //2013-7-19 cym add 显示原有滚动条
...
protected:
	...
	// fangz0602 仅仅单击表头的时候才画排序箭头
	BOOL		m_bNeedDrawSortArrow;
	
	// 
	bool32		m_bBlindKeyFocusCell;
	// xl 0807 虚模式下扫描blink需要额外开销，所以处理下blink
	BOOL		m_bEnableBlink;

	BOOL		m_bAutoHideFragmentaryCell;		// 隐藏只能显示一部分的单元格

	BOOL		m_bShowSelectWhenLoseFocus;		// 失去焦点时，是否显示选择

	BOOL		m_bDrawFixedCellGridLineAsNormal;	// 将固定格子的表格线绘制为一般表格样式，or以前的固定样式

	DWORD		m_dwDonotScrollToNonExistCell;	// 禁止滚动到不存在的表格处, 禁止方向可以设置，无则不禁止

	BOOL		m_bInitSortAscending;

	BOOL		m_bRowHeightFixed;				// 行高度将使用固定高度
...
//	BOOL		m_bShowGridLine;
	...
	bool		m_bEnablePolygonCorner;		//	启用多边形四角
    // Fonts and images
...
public:
	...
	DWORD		m_nDrawSelectedCellStyle;		// 绘制选择单元的方式 0-默认，以前的阴影背景方式 1-下边一个高亮线的方式

	BOOL		m_bRemoveSelectClickBlank;		// 点击空白区域取消上次选中的行
	BOOL		m_bListHeaderCanNotClick;		// 表头可以被选中
private:
