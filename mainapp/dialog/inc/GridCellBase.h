...
enum E_DrawMarkFlag
{
	EDMF_None = 0,		// 不绘制上下标
	EDMF_Top = 1,		// 绘制上标文字
	EDMF_Bottom = 2,	// 绘制下标文字
};

// 绘制上标，下标需要的数据
typedef struct _GV_DRAWMARK_ITEM
{
	E_DrawMarkFlag	eDrawMarkFlag;	// 绘制标记方法
	COLORREF		clrMark;		// 标记颜色
	CString			strMark;		// 标记文字
}GV_DRAWMARK_ITEM;
...
typedef struct _GV_ITEM {
	...
	CString  strTip;	  // 提示内容字串
	CString  strTipTitle; // 提示标题
	CRect	 rtTextPadding;	// 文本与边界间距
	GV_DRAWMARK_ITEM markItem;	// 标记文字
} GV_ITEM;
...
public:
	...
	virtual BOOL		InitDrawMarkFont(OUT CFont &FontMark);		// 初始化标记文字字体
...
public:
	...
	virtual CSize GetDrawMarkTextExtent(CDC *pDC);	// 获取标记文字的长度
...
protected:
	...
	int				m_iBlinkStep;
	float			m_fCompare;
	GV_DRAWMARK_ITEM	m_markItem;	// 标记数据, (&提示都全部放在基类了~)
	CString			m_StrTip;		// 提示
	CString			m_StrTipTitle;	// 提示标题
	HCURSOR			m_hCursor;		// 自定义的鼠标形状
	BOOL		m_bUnderLine;		// 是否要设置下划线
	CFont		m_font;
	bool		m_bVSeparatoLine;	// 是否要加垂直分割线
	bool		m_bHSeparatoLine;	// 是否要加水平分割线
