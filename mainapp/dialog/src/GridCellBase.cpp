...
CGridCellBase::CGridCellBase()
{
	...
	m_bVSeparatoLine = false;
	m_bHSeparatoLine = false;
}
...
void CGridCellBase::Reset()
{
	...
	m_markItem.eDrawMarkFlag = EDMF_None;
	m_markItem.clrMark = CLR_DEFAULT;
	m_markItem.strMark.Empty();
	m_rtTextPadding.SetRectEmpty();
	m_hCursor = NULL;
}
float CGridCellBase::GetCompareFloat()
{
	return m_fCompare;
}
void CGridCellBase::SetCompareFloat(float f)
{
	m_fCompare = f;
}
void CGridCellBase::operator=(const CGridCellBase& cell)
{
	...
	SetTextPadding(cell.m_rtTextPadding);
    SetText(cell.GetText());
	SetBlinkStep(cell.GetBlinkStep());
	...
	SetDrawMarkItem(cell.GetDrawMarkItem());
	SetTipTitle(GetTipTitle());
...
CSize CGridCellBase::GetDrawMarkTextExtent( CDC *pDC )
{
	const GV_DRAWMARK_ITEM &markItem = GetDrawMarkItem();
	if ( markItem.eDrawMarkFlag == EDMF_None
		|| markItem.strMark.IsEmpty() )
	{
		return CSize(0,0);
	}
	...
	const TCHAR *szText = markItem.strMark;
	...
	if ( !InitDrawMarkFont(FontMark) )		// 此处调用该函数会导致pDC中的font变更， 待解决这是为什么呢？因为获取静态的fontobj问题
	{
		pDC->SelectObject(pOldFont);
		return CSize(0, 0);
	}
...
{    
    ...
    // 加上标记所需要的部分, 仅考虑水平
	size.cx += GetDrawMarkTextExtent(pDC).cx;
    CSize ImageSize(0,0);    
...
BOOL CGridCellBase::InitDrawMarkFont( OUT CFont &FontMark )
{
    CFont *pFont = GetFontObject();
    if (!pFont)
	{
	...
	return FontMark.CreateFontIndirect(&lf);
}
