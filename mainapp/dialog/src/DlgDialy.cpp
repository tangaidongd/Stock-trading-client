...
auto_ptr<CDlgDialy> CDlgDialy::s_pDlg;		// 静态的对象
...
CString CDlgDialy::GetCurDlgTitle()
{
	...
	if ( EST_UserNote == m_eShowType )
	{
		...
		if ( AfxGetDocument() != NULL && AfxGetDocument()->m_pAbsCenterManager != NULL )
		{
			...
			{
				Str.Format(_T("投资日记 [%s-%s]"), pMerch->m_MerchInfo.m_StrMerchCnName, m_MerchCur.m_StrMerchCode);
			}
			else
			{
				Str.Format(_T("投资日记 [%s]"), m_MerchCur.m_StrMerchCode);
			}
		}
	}
	else if ( EST_InfoMine == m_eShowType )
	{
		...
		if ( AfxGetDocument() != NULL && AfxGetDocument()->m_pAbsCenterManager != NULL )
		{
			...
			if ( pAbsCenterManager->GetMerchManager().FindMerch(m_MerchCur.m_StrMerchCode, m_MerchCur.m_iMarketId, pMerch) )
			{
				...
				if ( m_bHistoryMine )
				{
					Str.Format(_T("资讯 [%s-%s]"), pMerch->m_MerchInfo.m_StrMerchCnName, m_MerchCur.m_StrMerchCode);
				}
				else
				{
					Str.Format(_T("资讯 [%s-%s]"), pMerch->m_MerchInfo.m_StrMerchCnName, m_MerchCur.m_StrMerchCode);
			...
			{
				Str.Format(_T("信息地雷 [%s]"), m_MerchCur.m_StrMerchCode);
			}
		}
	}
	else
