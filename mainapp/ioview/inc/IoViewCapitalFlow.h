#ifndef _IO_VIEW_CAPITALFLOW_H_
#define _IO_VIEW_CAPITALFLOW_H_
...
//////////////////////////////////////////////////////////////////////////
// 假设资金流向具有这些数据
struct T_CapitalFlowDataPseudo 
{
	...
	T_CapitalFlowDataPseudo()
	{
		memset(this, 0, sizeof(T_CapitalFlowDataPseudo));		// 浮点数0表示为0 000.... 伪结构简单话
	}

	bool32 operator == (const T_CapitalFlowDataPseudo &cap) const 
	{
		if ( this == &cap )
		{
			return true;
	...
	const T_CapitalFlowDataPseudo &operator+=(const T_CapitalFlowDataPseudo &cap);
	const T_CapitalFlowDataPseudo operator+(const T_CapitalFlowDataPseudo &cap);
...
class CIoViewCapitalFlow : public CIoViewBase
{
// Construction
public:
	CIoViewCapitalFlow();
	virtual ~CIoViewCapitalFlow();

	DECLARE_DYNCREATE(CIoViewCapitalFlow)
...
private:
	...
	typedef CArray<T_CapitalFlowDataPseudo, const T_CapitalFlowDataPseudo &> ShowDataArray;

	void		RequestData(CMmiCommBase &req);

	CString		GetCycleString();

	void		DrawHorizontalVolItem(CDC &dc, const CRect &rcItem, const CString &StrHeader, float fValue, float fMax, COLORREF clr);
	void		DrawVerticalAmountItem(CDC &dc, const CRect &rcItem, int32 iCenter, const CString &StrHeader, float fValue, float fMax, COLORREF clr);

	void		RecalcRectSize();

	void		UpdateShowData(const T_CapitalFlowDataPseudo &cap);		// 不应该直接调用
	void		UpdateShowData();		// 根据当前周期，提取合适的数据
	void		EmptyShowData();

	void		ChangeCycleType(E_Cycle eCycle);

	void		TickExToShowData(const T_TickEx &tickEx, OUT T_CapitalFlowDataPseudo &showData);
...
private:
	...
	T_CapitalFlowDataPseudo		m_capData;		// 资金流向数据 - 显示数据
