...
inline int Base64Encode(char * base64code, const char * src, int src_len = -1); 
inline int Base64Decode(char * buf, const char * base64code, int src_len = -1);
int ph_strlen(const char *pszStr);
void *ph_memcpy(void *pDest, const void *pSrc, int iCopyLen);
...
void *ph_memcpy(void *pDest,const void *pSrc, int iCopyLen)
{
    const char *ptmpSrc = (char *)pSrc;
    char *ptmpDest = (char *)pDest;
    if (pSrc < pDest)
    {
        for(int i=iCopyLen-1; i>=0; --i)
        {
            ptmpDest[i] = ptmpSrc[i];
        }
    }
    else
    {
...
inline int Base64Encode(char * base64code, const char * src, int src_len) 
{
	...
	{
		unsigned long ulTmp;
		ph_memcpy(&ulTmp, psrc, sizeof(ulong));
		...
		ulong uDest = b0 | b1 << 8 | b2 << 16 | b3 << 24;
		ph_memcpy(p64, &uDest, sizeof(ulong));
		len += 4;
		p64  += 4; 
		psrc += 3;
	}
