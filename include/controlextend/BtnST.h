...
class CONTROL_EXPORT CButtonSTMain : public CButton
{
public:
    CButtonSTMain();
	~CButtonSTMain();
...
public:
	...
	BOOL  SetBkBitmap(int nResourceID,int nStateCount);
	...
	BOOL	m_bIsOffset;	//鼠标点击按钮是否发生偏移 默认为TRUE(偏移)  lcq add 20131210
protected:
    //{{AFX_MSG(CButtonSTMain)
...
private:
	...
	void CancelHover();
	//lint -sem(CButtonSTMain::FreeResources,cleanup)
	...
	void DrawBkBitmap4(CDC *pDC);
	void DrawBkBitmap2(CDC *pDC);
