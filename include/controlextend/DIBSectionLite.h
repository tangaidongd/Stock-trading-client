...
class CONTROL_EXPORT CDIBSectionLite : public CObject
{
// Construction
public:
	CDIBSectionLite();
	virtual ~CDIBSectionLite();
	//lint -sem(CDIBSectionLite::DeleteObject,cleanup)
	//lint -sem(CDIBSectionLite::DeleteObject,initializer)
    void DeleteObject();
// static helpers
public:
    static int BytesPerLine(int nWidth, int nBitsPerPixel);
    static int NumColorEntries(int nBitsPerPixel, int nCompression = BI_RGB, DWORD biClrUsed = 0);
...
public:
	...
	//变成灰度图
	BOOL MakeGray( BOOL bSetPalette = FALSE );

	BOOL    MarkColor(const COLORREF &clr);
	static COLORREF  GetMarkColor(const COLORREF clrOld,const COLORREF &clr,int nAlpha);

	RGBQUAD RGBtoRGBQUAD(COLORREF cr);
	COLORREF RGBQUADtoRGB (RGBQUAD c);

	BOOL CompoundDIB(int left,int top,CDIBSectionLite *pDestDib,int nAlpha);
	BOOL CompoundDIB(int left,int top,CDIBSectionLite *pDib,CDIBSectionLite *pDIBAlpha);

	BOOL Rotate(CDIBSectionLite *pImagData , float angle );
// Operations (Palette)
public:
...
public:
    ...
    BOOL SetBitmapExt(LPBITMAPINFO lpBitmapInfo, LPVOID lpBits);   
	...
	BOOL Create(DWORD dwWidth, DWORD dwHeight, long wBpp, long imagetype =0 );
...
public:
	...
	static Status DrawImageGp(CDC *pDC, Image* pImage, int x,int y,int nsrcx,int nsrcy,int nsrcw,int nsrch);
	static Status DrawImageGp(CDC *pDC, Image* pImage, CRect rcDes,int nsrcx,int nsrcy,int nsrcw,int nsrch);
	static Status DrawImageGp(CDC *pDC, Image* pImage, CRect rcShow);
	static Status DrawImageGp(Graphics*, Image*, Rect *, GraphicsPath*,
				  ImageAttributes *pIab = NULL);

	static void DrawBlt(CDC* pDC, int nX, int nY, 
						 int nWidth,int nHeight, int nXSrc, int nYSrc,DWORD dwRop,CBitmap *bit);

	static void DrawStretchDIB(CDC* pDC, int nX, int nY, 
						 int nWidth,int nHeight, int nXSrc, int nYSrc,
						 int nWidthSrc,int nHeightSrc,DWORD dwRop,CBitmap *bit);

	static void TransparentBlt( CDC *pDC,      // 目标DC 
   ...
   const CDC* GetMemoryDC(CDC* pDC = NULL, BOOL bSelectPalette = TRUE);
